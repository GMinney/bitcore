...
const TransactionHash: React.FC = () => {
  ...
  const [confInterval, setConfInterval] = useState<NodeJS.Timeout | null>(null);
  ...
  useEffect(() => {
    ...
    Promise.all([fetcher(`${baseUrl}/tx/${tx}`), fetcher(`${baseUrl}/block/tip`), fetcher(`${baseUrl}/tx/${tx}/coins`)])
  ...
  }, [network, currency, tx, confInterval, dispatch, transaction]);
        ...
        <>
          ...
          {transaction && currency && network ? (
            <motion.div variants={routerFadeIn} animate="animate" initial="initial">
                ...
                (transaction.replacedByTxid ? (
                  ...
                  <Info
                    message={`This transaction was replaced by another transaction that ${
                      transaction.chain === 'ETH' ? 'used the same nonce' : 'spent some of its inputs'
                    }.`}
                    type={'error'}
                  />
                ))}
              ...
              <TransactionTileBody>
                <TransactionBodyCol $type="Twelve" $backgroundColor="transparent" $padding="0 0 1rem 0">
                  {currency === 'ETH' ? (
                  ...
                  <Tile $withBorderBottom>
                    ...
                    <TileDescription $value $textAlign="right">
                      <Link to={`/${currency}/${network}/block/${transaction.blockHash}`}>{transaction.blockHash}</Link>
                      {transaction.confirmations === -3 && (
                        <ConfirmationLabel padding="0 0 0 .5rem">Invalid</ConfirmationLabel>
                      )}
                      {transaction.confirmations === -1 && (
                        <ConfirmationLabel padding="0 0 0 .5rem">Unconfirmed</ConfirmationLabel>
                      )}
                    </TileDescription>
                  </Tile>
                </TransactionBodyCol>
              ...
              {currency === 'ETH' ? (
                <TransactionDetailsEth transaction={transaction} currency={currency} network={network} />
              ) : (
                <TransactionDetails
