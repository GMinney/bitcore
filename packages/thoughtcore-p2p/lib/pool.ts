...
import sha256  from 'thoughtcore-lib';
import { Peer } from './peer';
import Networks from 'thoughtcore-lib';
...
export class Pool extends EventEmitter {
  ...
  network: any;
  relay: boolean;
  static MaxConnectedPeers: number = 8;
  static RetrySeconds: number = 30;
  static PeerEvents: string[] = ['version', 'verack', 'addr', 'inv', 'getdata', 'merkleblock', 'getblocks', 'getheaders',
 ...
 */
numberConnected = function numberConnected() {
 ...
 */
_fillConnections = function _fillConnections() {
 ...
 */
_removeConnectedPeer = function _removeConnectedPeer(addr) {
 ...
 * @param {Object} addr - An addr from the list of addrs
 */
_connectPeer = function _connectPeer(addr) {
  ...
  if (!this._connectedPeers[addr.hash]) {
    var port = addr.port || this.network.port;
 ...
 */
_addConnectedPeer = function _addConnectedPeer(socket, addr) {
 ...
 */
_addPeerEventHandlers = function (peer, addr) {
 ...
 * @param {Object} addr - An addr from the list of addrs
 */
_deprioritizeAddr = function _deprioritizeAddr(addr) {
 ...
 * @param {Object}
 */
_addAddr = function _addAddr(addr) {
 ...
 * @param {Function} done
 */
_addAddrsFromSeed = function _addAddrsFromSeed(seed) {
 ...
 * @param {Function} done
 */
_addAddrsFromSeeds = function _addAddrsFromSeeds() {
