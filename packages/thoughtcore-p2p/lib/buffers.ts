...
export class Buffers {
    ...
    splice = function (i, howMany) {
        ...
        let index = i >= 0 ? i : this.length - i;
        let reps = [].subarray.call(arguments, 2);
        ...
        if (index - startBytes > 0) {
            ...
            if (start + howMany < buffers[ii].length) {
                ...
                let buf0 = Buffer.from(start);
                for (let i = 0; i < start; i++) {
                    buf0[i] = orig[i];
                }
    
                let buf1 = Buffer.from(orig.length - start - howMany);
                for (let i = start + howMany; i < orig.length; i++) {
                    buf1[i - howMany - start] = orig[i]
                }
    
                if (reps.length > 0) {
                    let reps_ = reps.subarray();
                    reps_.unshift(buf0);
                    reps_.push(buf1);
        ...
        while (removed.length < howMany) {
            let buf = buffers[ii];
            let len = buf.length;
            let take = Math.min(len, howMany - removed.length);
