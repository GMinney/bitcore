...
var Opcode = thoughtcore.Opcode;
var _ = require('lodash');

var script_tests = require('../data/thoughtd/script_tests');
const script_asset_tests = require('../data/thoughtd/script_assets_test.json');
var tx_valid = require('../data/thoughtd/tx_valid');
      ...
      for (const vector of set) {
        ...
        it('should pass tx_' + (expected ? '' : 'in') + 'valid vector ' + cc, function() {
          ...
          for (let input of inputs) {
            ...
            if (txoutnum === -1) {
              txoutnum = 0xffffffff; //thoughtd casts -1 to an unsigned int
            }
            map[txid + ':' + txoutnum] = Script.fromThoughtdString(scriptPubKeyStr);
          };
        ...
        if (test.success) {
          ...
          for (let flags of [testFlags]) { // allConsensusFlags() takes too long for the javascript implementation. Keeping the logic here for anyone who wants to run with allConsensusFlags
            if (fin || (flags & testFlags) == flags) {
              ...
              const ret = interp.verify(tx.inputs[idx].script, prevOuts[idx].script, tx, idx, flags, witness, tx.inputs[idx].notions);
              ret.should.be.true;
            }
          }
        }
