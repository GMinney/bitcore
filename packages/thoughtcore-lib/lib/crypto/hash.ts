...
export default class Hash {
  ...
  static sha1.blocksize = 512;
  static sha256.blocksize = 512;
  static sha512.blocksize = 1024;

  constructor() {}

  static sha1 = function (buf) {
    $.checkArgument(BufferUtil.isBuffer(buf));
    return crypto.createHash('sha1').update(buf).digest();
  };

  static sha256 = function (buf) {
    $.checkArgument(BufferUtil.isBuffer(buf));
  ...
  static sha512 = function (buf) {
    $.checkArgument(BufferUtil.isBuffer(buf));
    return crypto.createHash('sha512').update(buf).digest();
  };

  static hmac = function (hashf, data, key) {
  ...
  static sha256hmac = function (data, key) {
    return Hash.hmac(Hash.sha256, data, key);
  };

  static sha512hmac = function (data, key) {
