...
import SighashSchnorr from "./sighashschnorr.js";
...
export class Transaction {
  ...
  CURRENT_VERSION = 2; // mutable
  DEFAULT_NLOCKTIME = 0; // mutable
  MAX_BLOCK_SIZE = 4000000; // mutable
  ...
  // max amount of notions in circulation
  ...
  // Value used for fee estimation (notions per kilobyte)
  static FEE_PER_KB = 100000;

  // Safe upper bound for change address script size in bytes
  static CHANGE_OUTPUT_MAX_SIZE = 20 + 4 + 34 + 4;
  static MAXIMUM_EXTRA_SIZE = 4 + 9 + 9 + 4;

  constructor(serialized, opts) {
   ...
   * @return {Transaction}
   */
  shallowCopy = function (transaction) {
   ...
   * @return {Buffer}
   */
  _getWitnessHash = function () {
    return Hash.sha256sha256(this.toBuffer(false));
  };

  /**
   * Retrieve a hexa string that can be used with thoughtd's CLI interface
   ...
   * * `disableMoreOutputThanInput`: disable checking if the transaction spends more thoughts than the sum of the input amounts
   * @return {string}
   */
  serialize = function (unsafe) {
  ...
  uncheckedSerialize = toString = function () {
   ...
   * @return {string}
   */
  checkedSerialize = function (opts) {
    var serializationError = this.getSerializationError(opts);
    if (serializationError) {
      serializationError.message += ' - For more information please see: ' +
        'https://github.com/thoughtnetwork/thoughtcore/blob/master/packages/thoughtcore-lib/docs/transaction.md#serialization-checks';
      throw serializationError;
    }
    return this.uncheckedSerialize();
  };

  invalidNotions = function () {
    var invalid = false;
    for (var i = 0; i < this.outputs.length; i++) {
      if (this.outputs[i].invalidNotions()) {
        invalid = true;
      }
    }
    return invalid;
   ...
   *
   * @param {Object} opts allows to skip certain tests. {@see Transaction#serialize}
   * @return {thoughtcore.Error}
   */
  getSerializationError = function (opts) {
    opts = opts || {};

    if (this.invalidNotions()) {
      return new errors.Transaction.InvalidNotions();
    }
  ...
  _hasFeeError = function (opts, unspent) {
  ...
  _missingChange = function () {
    return !this._changeScript;
  };

  _hasDustOutputs = function (opts) {
    ...
    for (index in this.outputs) {
      ...
      if (output.notions < Transaction.DUST_AMOUNT && !output.script.isDataOut()) {
        return new errors.Transaction.DustOutputs();
      }
    }
  };

  _isMissingSignatures = function (opts) {
  ...
  toBufferWriter = function (writer, noWitness) {
  ...
  fromBufferReader = function (reader) {
  ...
  fromObject = function fromObject(arg, opts) {
  ...
  _checkConsistency = function (arg) {
    if (this._changeIndex != null) {
   ...
   * @return {Transaction} this
   */
  lockUntilBlockHeight = function (height) {
  ...
  _newTransaction = function () {
    ...
    this.nLockTime = this.DEFAULT_NLOCKTIME;
  };
   ...
   *
   * Can receive, as output information, the output of thoughtd's `listunspent` command,
   * and a slightly fancier format recognized by thoughtcore:
   *
   * ```
   * {
   ...
   * }
   * ```
   * Where `address` can be either a string or a thoughtcore Address object. The
   * same is true for `script`, which can be a string or a thoughtcore Script.
   *
   ...
   *
   * // From a pay to public key hash output from thoughtd's listunspent
   * transaction.from({'txid': '0000...', vout: 0, amount: 0.1, scriptPubKey: 'OP_DUP ...'});
   *
   * // From a pay to public key hash output
   * transaction.from({'txId': '0000...', outputIndex: 0, notions: 1000, script: 'OP_DUP ...'});
   *
   * // From a multisig P2SH output
   * transaction.from({'txId': '0000...', inputIndex: 0, notions: 1000, script: '... OP_HASH'},
   *                  ['03000...', '02000...'], 2);
   * ```
   *
   ...
   */
  from = function (utxo, pubkeys, threshold, opts) {
   ...
   */
  associateInputs = function (utxos, pubkeys, threshold, opts = {}) {
    let indexes = [];
  ...
  _selectInputType = function (utxo, pubkeys, threshold) {
    var clazz;
  ...
  _getInputFrom = function (utxo, pubkeys, threshold, opts = {}) {
    utxo = new UnspentOutput(utxo);
    const InputClass = this._selectInputType(utxo, pubkeys, threshold);
    const input = {
      output: new Output({
        script: utxo.script,
        notions: utxo.notions
      }),
      prevTxId: utxo.txId,
  ...
  _fromNonP2SH = function (utxo, opts) {
    const input = this._getInputFrom(utxo, null, null, opts);
    this.addInput(input);
  };

  _fromMultisigUtxo = function (utxo, pubkeys, threshold, opts) {
   ...
   * set, two additional parameters, `outputScript` and `notions` can be provided.
   *
   * @param {Input} input
   ...
   */
  addInput = function (input, outputScript, notions) {
    $.checkArgumentType(input, Input, 'input');
    if (!input.output && (outputScript == null || notions == null)) {
      throw new errors.Transaction.NeedMoreInfo('Need information about the UTXO script and notions');
    }
    if (!input.output && outputScript && notions != null) {
      outputScript = outputScript instanceof Script ? outputScript : new Script(outputScript);
      $.checkArgumentType(notions, 'number', 'notions');
   ...
   * @return Transaction this, for chaining
   */
  uncheckedAddInput = function (input) {
    $.checkArgumentType(input, Input, 'input');
    this.inputs.push(input);
    this._inputAmount = undefined;
   ...
   * be reset).
   *
   * @param {number} amount notions to be sent
   * @return {Transaction} this, for chaining
   */
  fee = function (amount) {
   ...
   * be reset).
   *
   * @param {number} amount notions per KB to be sent
   * @return {Transaction} this, for chaining
   */
  feePerKb = function (amount) {
    $.checkArgument(!isNaN(amount), 'amount must be a number');
   ...
   *
   * @param {number} amount notions per Byte to be sent
   * @return {Transaction} this, for chaining
   */
  feePerByte = function (amount) {
    $.checkArgument(!isNaN(amount), 'amount must be a number');
   ...
   * @param {number} amount in notions
   * @return {Transaction} this, for chaining
   */
  to = function (address, amount) {
    if (Array.isArray(address)) {
      for (const to of address) {
        this.to(to.address, to.notions);
      }
      return this;
    }
  ...
  /**
   * Calculates or gets the total output amount in notions
   *
  ...
  _getOutputAmount = function () {
    if (this._outputAmount == null) {
      ...
      for (const output of this.outputs || []) {
        self._outputAmount += output.notions;
      }
    }
  ...
  /**
   * Calculates or gets the total input amount in notions
   *
  ...
  _getInputAmount = function () {
    if (this._inputAmount == null) {
      this._inputAmount = _.sumBy(this.inputs, function (input) {
        ...
        return input.output.notions;
      });
    }
    return this._inputAmount;
  };

  _updateChangeOutput = function (noClearSigs) {
    if (!this._changeScript) {
      return;
    }
    if (!noClearSigs) {
   ...
   *
   * @return {Number} fee of this transaction in notions
   */
  getFee = function () {
    if (this.isCoinbase()) {
      return 0;
  ...
  _estimateSizeOfChangeOutput = function () {
    if (!this._changeScript) {
      return 0;
    }
    const scriptLen = this._changeScript.toBuffer().length;
    // 8 bytes for notions + script size + actual script size
  ...
  sort = function () {
    ...
    this.sortOutputs(function (outputs) {
      ...
      copy.forEach((x) => { x.i = i++ });
      copy.sort(function (first, second) {
        return first.notions - second.notions
  ...
  _newOutputOrder = function (newOutputs) {
  ...
  removeInput = function (txId, outputIndex) {
    var index;
   ...
   * @return {Transaction} this, for chaining
   */
  sign = function (privateKey, sigtype, signingMethod, merkleRoot) {
  ...
  getSignatures = function (privKey, sigtype, signingMethod, merkleRoot) {
   ...
   * @return {Transaction} this, for chaining
   */
  applySignature = function (signature, signingMethod) {
  ...
  isValidSignature = function (signature, signingMethod) {
   ...
   * @returns {Boolean}
   */
  checkEcdsaSignature = function (sig, pubkey, nin, subscript, notions) {
    ...
    var notionsBuffer;
    if (notions) {
      $.checkState(JSUtil.isNaturalNumber(notions), 'notions needs to be a natural number');
      notionsBuffer = new BufferWriter().writeUInt64LEBN(new BN(notions)).toBuffer();
    } else {
      notionsBuffer = this.inputs[nin].getNotionsBuffer();
    }
   ...
   * @param {Number} notions ECDSA only
   * @param {Object} execdata Schnorr only
   * @returns {Boolean} whether the signature is valid for this transaction input
   */
  verifySignature = function (sig, pubkey, nin, subscript, sigversion, notions, execdata) {
    if (sigversion == null) {
    ...
    switch (sigversion) {
      case Signature.Version.WITNESS_V0:
        return this.checkEcdsaSignature(sig, pubkey, nin, subscript, notions);
   ...
   * CheckTransaction in thought core.
   */
  verify = function () {
    ...
    for (var i = 0; i < this.outputs.length; i++) {
      ...
      if (txout.invalidNotions()) {
        return 'transaction txout ' + i + ' notions is invalid';
      }
      if (txout._notionsBN.gt(new BN(Transaction.MAX_MONEY, 10))) {
        return 'transaction txout ' + i + ' greater than MAX_MONEY';
      }
      valueoutbn = valueoutbn.add(txout._notionsBN);
    ...
    // Size limits
    if (this.toBuffer().length > this.MAX_BLOCK_SIZE) {
  ...
  /**
   * Analogous to thoughtd's IsCoinBase function in transaction.h
   */
  isCoinbase = function () {
  ...
  setVersion = function (version) {
    $.checkArgument(
      JSUtil.isNaturalNumber(version) && version <= this.CURRENT_VERSION,
