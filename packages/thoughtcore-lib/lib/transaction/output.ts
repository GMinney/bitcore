...
export class Output {
  ...
  invalidNotions = function () {
    ...
    if (this._notions !== this._notionsBN.toNumber()) {
      return 'transaction txout notions has corrupted value';
    }
    if (this._notions < 0) {
  ...
  setScriptFromBuffer = function (buffer) {
  ...
  toBufferWriter = function (writer) {
    ...
    writer.writeUInt64LEBN(this._notionsBN);
   ...
   */
  add = function (depth, script, leafVersion, track = true) {
  ...
  _insertNode = function (node, depth) {
...
Object.defineProperty(Output.prototype, 'notions', {
  ...
  set: function (num) {
    if (num instanceof BN) {
      ...
      this._notions = parseInt(num);
      this._notionsBN = BN.fromNumber(this._notions);
    } else {
      $.checkArgument(
        JSUtil.isNaturalNumber(num),
        'Output notions is not a natural number'
      );
      this._notionsBN = BN.fromNumber(num);
      this._notions = num;
    }
    $.checkState(
      JSUtil.isNaturalNumber(this._notions),
      'Output notions is not a natural number'
    );
  }
});
