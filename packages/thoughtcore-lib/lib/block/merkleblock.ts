 ...
 */
validMerkleTree = function validMerkleTree() {
 ...
 */
filterdTxsHash = function filterdTxsHash() {
    $.checkState(_.isArray(this.flags), 'MerkleBlock flags is not an array');
    $.checkState(_.isArray(this.hashes), 'MerkleBlock hashes is not an array');
 ...
 * Modeled after Thought Core merkleblock.cpp TraverseAndExtract()
 ...
 * @private
 */
_traverseMerkleTree = function traverseMerkleTree(depth, pos, opts, checkForTxs) {
    opts = opts || {};
    opts.txs = opts.txs || [];
 ...
 *  Modeled after Thought Core merkleblock.h CalcTreeWidth()
 * @param {Number} - Height at which we want the tree width
 * @returns {Number} - Width of the tree at a given height
 * @private
 */
_calcTreeWidth = function calcTreeWidth(height) {
 ...
 * @private
 */
_calcTreeHeight = function calcTreeHeight() {
    var height = 0;
 ...
 * @private
 */
hasTransaction = function hasTransaction(tx) {
