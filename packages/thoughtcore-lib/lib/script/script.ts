...
'use strict';
/**
 * A thought transaction script. Each transaction's inputs and outputs
 * has a script that is evaluated to validate it's spending.
 *
 * See https://en.thought.it/wiki/Script
 *
 * @constructor
...
export default class Script {
  ...
  static types = {
    ...
    CRIPTHASH_OUT: 'Pay to script hash',
  ...
  static OP_RETURN_STANDARD_SIZE = 80;
  ...
  constructor(from?) {
    ...
    if (BufferUtil.isBuffer(from)) {
      return this.fromBuffer(from);
    } else if (from instanceof Address) {
      return this.fromAddress(from);
    } else if (from instanceof Script) {
      return this.fromBuffer(from.toBuffer());
...
_chunkToString = function (chunk, type) {
  var opcodenum = chunk.opcodenum;
  var asm = (type === 'asm');
  var str = '';
  if (!chunk.buf) {
 ...
 */
static isPublicKeyHashOut = function () {
 ...
 */
static isPublicKeyHashIn = function () {
 ...
 */
static isPublicKeyOut = function () {
  if (this.chunks.length === 2 &&
    this.chunks[0].buf &&
 ...
 */
static isScriptHashOut = function () {
 ...
 */
isWitnessScriptHashOut = function () {
  var buf = this.toBuffer();
 ...
 */
isWitnessPublicKeyHashOut = function () {
  var buf = this.toBuffer();
 ...
 * @returns {boolean} if this is a p2wpkh output script
 */
isWitnessProgram = function (values) {
  if (!values) {
    values = {};
  }
  var buf = this.toBuffer();
 ...
 */
static isScriptHashIn = function () {
  if (this.chunks.length <= 1) {
    return false;
  }
  ...
  var redeemScript;
  try {
    redeemScript = this.fromBuffer(redeemBuf);
  } catch (e) {
...
_addByType = function (obj, prepend) {
...
_insertAtPosition = function (op, prepend) {
  if (prepend) {
...
_addOpcode = function (opcode, prepend) {
  var op;
...
_addBuffer = function (buf, prepend) {
  var opcodenum;
  var len = buf.length;
 ...
 */
buildMultisigOut = function (publicKeys, threshold, opts) {
...
buildWitnessMultisigOutFromScript = function (script) {
 ...
 * @returns {Script}
 */
buildMultisigIn = function (pubkeys, threshold, signatures, opts) {
 ...
 * @returns {Script}
 */
buildP2SHMultisigIn = function (pubkeys, threshold, signatures, opts) {
  $.checkArgument(Array.isArray(pubkeys));
  $.checkArgument(!isNaN(threshold));
  $.checkArgument(Array.isArray(signatures));
  opts = opts || {};
  ...
  s.add((opts.cachedMultisig || this.buildMultisigOut(pubkeys, threshold, opts)).toBuffer());
  return s;
};
 ...
 */
buildPublicKeyHashOut = function (to) {
 ...
 * @returns {Script}
 */
buildWitnessV1Out = function (to, scriptTree) {
  $.checkArgument(to instanceof PublicKey || to instanceof Address || typeof to === 'string');
 ...
 *  public key
 */
buildPublicKeyOut = function (pubkey) {
 ...
 */
buildDataOut = function (data, encoding) {
 ...
 */
buildScriptHashOut = function (script) {
 ...
 */
buildPublicKeyIn = function (signature, sigtype) {
 ...
 * @param {number=} sigtype - the type of the signature (defaults to SIGHASH_ALL)
 */
buildPublicKeyHashIn = function (publicKey, signature, sigtype) {
  $.checkArgument(signature instanceof Signature || BufferUtil.isBuffer(signature));
  $.checkArgument(sigtype == null || !isNaN(sigtype));
  if (signature instanceof Signature) {
    signature = signature.toBuffer();
 ...
 */
toScriptHashOut = function () {
  return this.buildScriptHashOut(this);
};
 ...
 * @private
 */
_getOutputAddressInfo = function () {
  var info = {hashBuffer: null, type: null};
...
/**
 * Analogous to thoughtd's FindAndDelete. Find and delete equivalent chunks,
...
/**
 * Comes from thoughtd's script GetSigOpCount(boolean) function
 * @param {boolean} use current (true) or pre-version-0.6 (false) logic
 * @returns {number} number of signature operations required by this script
 */
getSignatureOperationsCount = function (accurate) {
