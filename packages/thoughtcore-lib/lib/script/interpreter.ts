...
import SighashWitness from "../transaction/sighashwitness.js";
import SighashSchnorr from "../transaction/sighashschnorr.js";
import BufferWriter from "../encoding/bufferwriter.js";
import TaggedHash from "../crypto/taggedhash.js";

'use strict';
/**
 * Thought transactions contain scripts. Each input has a script called the
...
export default class Interpreter {
  ...
  static LOCKTIME_THRESHOLD_BN = new BN(Interpreter.LOCKTIME_THRESHOLD);

  // flags taken from thoughtd
  ...
  static TAPROOT_CONTROL_MAX_SIZE = Interpreter.TAPROOT_CONTROL_BASE_SIZE + Interpreter.TAPROOT_CONTROL_NODE_SIZE * Interpreter.TAPROOT_CONTROL_MAX_NODE_COUNT;

  // Conceptually, this doesn't really belong with the Interpreter, but I haven't found a better place for it.
  static PROTOCOL_VERSION = 70018;
  ...
  verifyWitnessProgram = function (version, program, witness, notions, flags, isP2SH) {
    ...
    if (version === 0) {
      if (program.length === Interpreter.WITNESS_V0_SCRIPTHASH_SIZE) {
        ...
        return this.executeWitnessScript(scriptPubKey, stack, Signature.Version.WITNESS_V0, notions, flags);
      ...
      if (stack.length === 1) {
        ...
        if ((control[0] & Interpreter.TAPROOT_LEAF_MASK) === Interpreter.TAPROOT_LEAF_TAPSCRIPT) {
          ...
          return this.executeWitnessScript(scriptPubKey, stack, Signature.Version.TAPSCRIPT, notions, flags, execdata);
        }
  ...
  executeWitnessScript = function (scriptPubKey, stack, sigversion, notions, flags, execdata) {
   ...
   * @param {number} notions - number of notions created by this output
   *
   * Translated from thoughtd's VerifyScript
   */
  verify = function (scriptSig, scriptPubkey, tx, nin, flags, witness, notions) {
    ...
    if ((flags & Interpreter.SCRIPT_VERIFY_WITNESS)) {
      ...
      if (scriptPubkey.isWitnessProgram(witnessValues)) {
        ...
        if (!this.verifyWitnessProgram(witnessValues.version, witnessValues.program, witness, notions, this.flags, /* isP2SH */ false)) {
          return false;
        }
      }
    }
    ...
    if ((flags & Interpreter.SCRIPT_VERIFY_P2SH) && scriptPubkey.isScriptHashOut()) {
      ...
      if ((flags & Interpreter.SCRIPT_VERIFY_WITNESS)) {
        ...
        if (redeemScript.isWitnessProgram(p2shWitnessValues)) {
          ...
          if (!this.verifyWitnessProgram(p2shWitnessValues.version, p2shWitnessValues.program, witness, notions, this.flags, /* isP2SH */ true)) {
            return false;
          }
  ...
  set = function (obj) {
    ...
    this.notions = typeof obj.notions === 'undefined' ? this.notions : obj.notions;
   ...
   */
  checkSignatureEncoding = function (buf) {
    var sig;
   ...
   */
  checkPubkeyEncoding = function (buf) {
  ...
  checkEcdsaSignature = function (sig, pubkey, nin, subscript, notions) {
    ...
    $.checkState(JSUtil.isNaturalNumber(notions));
    var notionsBuffer = new BufferWriter().writeUInt64LEBN(new BN(notions)).toBuffer();
   ...
   * @returns {Boolean}
   */
  checkSchnorrSignature = function (sig, pubkey, sigversion, execdata) {
   ...
   */
  _evalChecksigPreTapscript = function (bufSig, bufPubkey) {
    $.checkArgument(
    ...
    try {
      ...
      retVal.success = this.tx.verifySignature(sig, pubkey, this.nin, subscript, this.sigversion, this.notions);
    } catch (e) {
   ...
   * thoughtd commit: a0988140b71485ad12c3c3a4a9573f7c21b1eff8
   */
  _evalChecksigTapscript = function (bufSig, bufPubkey) {
  ...
  /**
   * Based on thoughtd's EvalChecksig function
   * thoughtd commit: a0988140b71485ad12c3c3a4a9573f7c21b1eff8
   * @returns {{ success: Boolean, verified: Boolean }}
   */
  _evalCheckSig = function (bufSig, bufPubkey) {
  ...
  /**
   * Based on thoughtd's EvalScript function, with the inner loop moved to
   ...
   *
   * See the corresponding code on thought core:
   * https://github.com/thought/thought/blob/ffd75adce01a78b3461b3ff05bcc2b530a9ce994/src/script/interpreter.cpp#L1129
   *
   * @param {BN} nLockTime the locktime read from the script
   * @return {boolean} true if the transaction's locktime is less than or equal to
   *                   the transaction's locktime
   */
  checkLockTime = function (nLockTime) {
   ...
   */
  checkSequence = function (nSequence) {
  ...
  computeTapleafHash = function (leafVersion, scriptBuf) {
  ...
  verifyTaprootCommitment = function (control, program, tapleafHash) {
        ...
        case Opcode.OP_CHECKMULTISIGVERIFY:
          {
            ...
            while (fSuccess && nSigsCount > 0) {
              ...
              try {
                ...
                fOk = this.tx.verifySignature(sig, pubkey, this.nin, subscript, this.sigversion, this.notions, this.execdata);
              } catch (e) {
                //invalid sig or pubkey
                fOk = false;
              }
