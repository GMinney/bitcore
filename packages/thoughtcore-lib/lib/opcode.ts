...
export default class Opcode {
   ...
   * @returns {Number}
   */
  decodeOpN = function (opcode) {
    if (opcode === Opcode.map.OP_0) {
      return 0;
    }
    $.checkArgument(opcode >= Opcode.map.OP_1 && opcode <= Opcode.map.OP_16, new Error('Invalid opcode: ' + JSON.stringify(opcode)));
    return opcode - (Opcode.map.OP_1 - 1);
  };

  /**
   * Returns true if given opcode is classified as a "success".
   * This was taken from commit https://github.com/thought/thought/commit/72422ce396b8eba7b1a72c171c2f07dae691d1b5
   * @param {Number|String} opcode 
   * @returns {Boolean}
   */
  isOpSuccess = function (opcode) {
