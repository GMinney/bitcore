 ...
 * var imported = fromString(exported);
 * ```
 *
 ...
 * @private
 */
_transformPrivateKey = function (privkey) {
    $.checkArgument(this._isPrivateKey(privkey), 'Must be an instance of PrivateKey');
    var info = {};
 ...
 * @private
 */
_transformDER = function (buf, strict) {
    /* jshint maxstatements: 30 */
    /* jshint maxcomplexity: 12 */
    $.checkArgument(_isBuffer(buf), 'Must be a hex buffer of DER encoded public key');
    var info = {};
    ...
    if (buf[0] === 0x04 || (!strict && (buf[0] === 0x06 || buf[0] === 0x07))) {
      ...
      xbuf = buf.subarray(1);
      x = new BN(xbuf);
      info = _transformX(true, x);
      info.compressed = true;
    } else if (buf[0] === 0x02) {
      xbuf = buf.subarray(1);
      x = new BN(xbuf);
      info = _transformX(false, x);
      info.compressed = true;
    } else {
 ...
 */
fromPrivateKey = function (privkey) {
    $.checkArgument(this._isPrivateKey(privkey), 'Must be an instance of PrivateKey');
    var info = this._transformPrivateKey(privkey);
 ...
 * @returns {PublicKey}
 */
fromBuffer = function (buf, strict) {
    $.checkArgument(this._isBuffer(buf), 'Must be a hex buffer of DER encoded public key or 32 byte X coordinate (taproot)');
    if (buf.length === 32) {
      return this.fromX(false, buf);
    }
 ...
 * @returns {PublicKey} A new valid instance of PublicKey
 */
fromDER = function (buf, strict) {
    $.checkArgument(this._isBuffer(buf), 'Must be a hex buffer of DER encoded public key');
    var info = this._transformDER(buf, strict);
    return new PublicKey(info.point, {
      compressed: info.compressed
    });
  };
 ...
 * @returns {PublicKey} A new valid instance of PublicKey
 */
fromPoint = function (point, compressed) {
 ...
 * @returns {PublicKey} A new valid instance of PublicKey
 */
fromString = function (str, encoding) {
    var buf = Buffer.from(str, encoding || 'hex');
    var info = this._transformDER(buf);
    return new PublicKey(info.point, {
      compressed: info.compressed
    });
  };
 ...
 * @returns {PublicKey} A new valid instance of PublicKey
 */
fromX = function (odd, x) {
    var info = this._transformX(odd, x);
    return new PublicKey(info.point, {
      compressed: info.compressed
    });
  };
...
fromTaproot = function (hexBuf) {
    ...
    return new this.fromX(false, hexBuf);
  }
 ...
 * @returns {Boolean}
 */
isValidTaproot = function (hexBuf) {
    try {
      return !!this.fromTaproot(hexBuf);
    } catch (e) {
      return false;
    }
  };
 ...
 * @param {Object} extra
 */
_classifyArgs = function (data, extra) {
    ...
    if (data instanceof Point) {
      ...
      info = this._transformDER(Buffer.from(data, 'hex'));
    } else if (this._isBuffer(data)) {
      info = this._transformDER(data);
    } else if (this._isPrivateKey(data)) {
      info = this._transformPrivateKey(data);
    } else {
 ...
 * @returns {Buffer}
 */
computeTapTweakHash = function (merkleRoot) {
 ...
 * @returns {Boolean}
 */
checkTapTweak = function (p, merkleRoot, control) {
    if (Buffer.isBuffer(p)) {
      p = this.fromTaproot(p);
    }
 ...
 * @see https://github.com/thought/thought/blob/master/src/pubkey.h#L141
 * @returns {Buffer}
 */
