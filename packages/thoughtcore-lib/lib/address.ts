...
import Bech32 from "./encoding/bech32.js";
...
/** @static */
static PayToWitnessPublicKeyHash = 'witnesspubkeyhash';
/** @static */
static PayToWitnessScriptHash = 'witnessscripthash';
/** @static */
static PayToTaproot = 'taproot';

  constructor(data, network, type, multisigType?) {
    if (!(this instanceof Address)) {
      return new Address(data, network, type);
    }
  
    if (_.isArray(data) && _.isNumber(network)) {
      return this.createMultisig(data, network, type, false, multisigType);
    }
 ...
 * @private
 */
_transformHash = function (hash, network, type) {
  var info = {};
 ...
 * @return {Address}
 */
_transformObject = function (data) {
 ...
 * @private
 */
_classifyFromVersion = function (buffer) {
  var version = {};
...
/**
 * Internal function to transform a thought address buffer
 *
 * @param {Buffer} buffer - An instance of a hex encoded address Buffer
 ...
 * @private
 */
_transformBuffer = function (buffer, network, type) {
  ...
  var bufferVersion = this._classifyFromVersion(buffer);
 ...
 * @private
 */
_transformPublicKey = function (pubkey, network, type) {
  var info = {};
 ...
 * @private
 */
_transformScript = function (script, network) {
 ...
 *
 * The addresses will be sorted lexicographically, as that is the trend in thought.
 ...
 * @return {Address}
 */
createMultisig = function (publicKeys, threshold, network, nestedWitness, type) {
  ...
  if (nestedWitness) {
    return this.payingTo(Script.buildWitnessMultisigOutFromScript(redeemScript), network);
  }
  return this.payingTo(redeemScript, network, type);
};

/**
 * Internal function to transform a thought address string
 *
 * @param {string} data
 * @param {String|Network=} network - either a Network instance, 'livenet', or 'testnet'
 * @param {string=} type - The type: 'pubkeyhash', 'scripthash', 'witnesspubkeyhash', or 'witnessscripthash'
 ...
 * @private
 */
_transformString = function (data, network, type) {
  ...
  try {
    var info = this._transformBuffer(Buffer.from(data, 'utf8'), network, type);
    return info;
  } catch (e) {
  ...
  var info = this._transformBuffer(addressBuffer, network, type);
  return info;
};
 ...
 * @returns {Address} A new valid and frozen instance of an Address
 */
fromPublicKey = function (data, network, type) {
  var info = this._transformPublicKey(data, network, type);
 ...
 * @returns {Address} A new valid and frozen instance of an Address
 */
fromPublicKeyHash = function (hash, network) {
 ...
 * @returns {Address} A new valid and frozen instance of an Address
 */
fromScriptHash = function (hash, network, type) {
 ...
 * @returns {Address} A new valid and frozen instance of an Address
 */
payingTo = function (script, network, type) {
  ...
  var type = type || Address.PayToScriptHash;
  return this.fromScriptHash(hash, network, type);
};
 ...
 * @returns {Address} A new valid and frozen instance of an Address
 */
fromScript = function (script, network) {
  $.checkArgument(script instanceof Script, 'script must be a Script instance');
 ...
 * @returns {Address} A new valid and frozen instance of an Address
 */
fromBuffer = function (buffer, network, type) {
 ...
 *
 * @param {string} str - An string of the thought address
 * @param {String|Network=} network - either a Network instance, 'livenet', or 'testnet'
 * @param {string=} type - The type of address: 'script' or 'pubkey'
 * @returns {Address} A new valid and frozen instance of an Address
 */
fromString = function (str, network, type) {
  var info = this._transformString(str, network, type);
  return new Address(info.hashBuffer, info.network, info.type);
};
 ...
 */
getValidationError = function (data, network, type) {
  var error;
  try {
    /* jshint nonew: false */
 ...
 */
isValid = function (data, network, type) {
 ...
 * @return boolean
 */
isPayToWitnessPublicKeyHash = function () {
 ...
 *
 * @returns {Buffer} Thought address buffer
 */
toBuffer = function () {
 ...
 *
 * @returns {string} Thought address
 */
toString = function () {
