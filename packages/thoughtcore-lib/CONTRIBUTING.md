# Contributing to Thoughtcore

We're working hard to make *thoughtcore* the most powerful JavaScript library for working with thought. Our goal is to have *thoughtcore* be a library that can be used by anyone interested in thought, and to level expertise differences with great design and documentation.

## Community

If there are any questions, etc., please feel to ask in one of the community channels:

- [Support Forum](https://labs.thoughtnetwork.com/c/thoughtcore)
- [Development Chat](https://gitter.im/thoughtnetwork/thoughtcore)
...
These are some global design goals in thoughtcore that any change must adhere.
...
#### G3 - Internal Amounts Must be Integers Representing Notions

Avoid representation errors by always dealing with notions. For conversion for frontends, use the `Unit` class.
...
When possible, thoughtcore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Thoughtcore methods. For example, calling `JSON.stringify` on an Thoughtcore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'thoughtcore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the thoughtcore object.

DO:

```javascript
var thoughtcore = require('../');
var PublicKey = thoughtcore.PublicKey;
```
...
```sh
git remote add thoughtnetwork git@github.com:thoughtnetwork/thoughtcore.git
git pull --rebase thoughtnetwork master
```
...
Finally go to [github.com/thoughtnetwork/thoughtcore](https://github.com/thoughtnetwork/thoughtcore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of thoughtcore.

If you have any questions feel free to post them to
[github.com/thoughtnetwork/thoughtcore/issues](https://github.com/thoughtnetwork/thoughtcore/issues).
