...
var thoughtjs = require('thoughtjs-lib');
...
async.series([
  function (next) {
    ...
    for (var i = 0; i < 100; i++) {
      ...
      // uint64le
      var br = new thoughtcore.encoding.BufferWriter();
      var num = Math.round(Math.random() * 10000000000000);
      br.writeUInt64LEBN(new thoughtcore.crypto.BN(num));
      buffers.push(br.toBuffer());

      // hashes
      var data = thoughtcore.crypto.Hash.sha256sha256(new Buffer(32));
    ...
    function readUInt64LEBN() {
      ...
      var buf = buffers[c];
      var br = new thoughtcore.encoding.BufferReader(buf);
  ...
  function (next) {
    ...
    function thoughtcoreTest() {
      block1 = thoughtcore.Block.fromString(blockData);
    }

    function thoughtJSTest() {
      block2 = thoughtjs.Block.fromHex(blockData);
    }
