...
const thoughtcoreLib = require('crypto-wallet-core').ThoughtcoreLib;

export class Storage {
  path: string;
  db: Array<Mongo | Level | TextFile>;
  collection: 'thoughtcoreWallets';
  url?: string;
  ...
  async addKeys(params: { name: string; keys: KeyImport[]; encryptionKey: string }) {
    ...
    for (const key of keys) {
      let { pubKey, path } = key;
      pubKey = pubKey || new thoughtcoreLib.PrivateKey(key.privKey).publicKey.toString();
      let payload = {};
