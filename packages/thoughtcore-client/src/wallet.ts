...
const Mnemonic = require('thoughtcore-mnemonic');
const { ParseApiStream } = require('./stream-util');

const { PrivateKey, HDPrivateKey } = ThoughtcoreLib;
const chainLibs = {
  THT: ThoughtcoreLib,
  ETH: { Web3, ethers },
  MATIC: { Web3, ethers },
...
export class Wallet {
  ...
  constructor(params: Wallet | WalletObj) {
    ...
    if (!this.baseUrl) {
      this.baseUrl = 'https://api.thoughtcore.io/api';
    }
  ...
  static async create(params: Partial<WalletObj>) {
    ...
    // save wallet to storage and then thoughtcore-node
    ...
    await loadedWallet.register().catch(e => {
      console.debug(e);
      console.error('Failed to register wallet with thoughtcore-node.');
    });
  ...
  isUtxoChain() {
    ...
    return ['THT', 'BCH', 'DOGE', 'LTC'].includes(this.chain?.toUpperCase() || 'THT');
  }
  ...
  async signTx(params) {
    ...
    if (!keys && !signingKeys) {
      ...
      let decryptedParams = Encryption.thoughtCoreDecrypt(addresses, passphrase);
