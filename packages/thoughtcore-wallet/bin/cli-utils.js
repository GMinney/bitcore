...
Utils.getClient = function (args, opts, cb) {
  ...
  var host = args.host || 'https://bws.thoughtnetwork.com/';
...
Utils.configureCommander = function (program) {
  program
    ...
    .option('-h, --host <host>', 'Thoughtcore Wallet Service URL (eg: http://localhost:3232/bws/api ')
...
Utils.renderAmount = function (notions, coin, opts) {
  ...
  var coin = coin || 'tht';
  var u = Utils.COIN[coin] || Utils.COIN.tht;
  var amount = clipDecimals((notions / u.toNotions), u.maxDecimals).toFixed(u.maxDecimals);
