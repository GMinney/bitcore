...
export class THTTxProvider {
  ...
  create({ recipients, utxos = [], change, feeRate, fee, isSweep, replaceByFee, lockUntilDate, lockUntilBlock }) {
    ...
    const thtUtxos = filteredUtxos.map(utxo => {
      const thtUtxo = Object.assign({}, utxo, {
        amount: utxo.value / 1e8,
        txid: utxo.mintTxid,
        outputIndex: utxo.mintIndex
      });
      return new this.lib.Transaction.UnspentOutput(thtUtxo);
    });
    let tx = new this.lib.Transaction().from(thtUtxos);
    if (fee) {
      tx.fee(fee);
    }
    if (feeRate) {
  ...
  getHash(params: { tx: string }) {
    const thoughtcoreTx = new this.lib.Transaction(params.tx);
    return thoughtcoreTx.hash;
  }
  ...
  sign(params: { tx: string; keys: Array<Key>; utxos: any[]; pubkeys?: any[]; threshold?: number; opts: any }) {
    ...
    let thoughtcoreTx = new this.lib.Transaction(tx);
    let applicableUtxos = this.getRelatedUtxos({
      outputs: thoughtcoreTx.inputs,
      utxos
    });
    thoughtcoreTx.associateInputs(applicableUtxos, pubkeys, threshold, opts);
    const privKeys = _.uniq(keys.map(key => key.privKey.toString()));
    const signedTx = thoughtcoreTx.sign(privKeys).toString();
    return signedTx;
  }
  ...
  getOutputsFromTx({ tx }) {
    return tx.outputs.map(({ script, notions }) => {
      let address = script;
      return { address, notions };
    });
  }
