...
describe('Address Derivation', () => {
  it('should be able to generate a valid THT address', () => {
    ...
    const path = Deriver.pathFor('THT', 'main');
    expect(path).to.equal("m/44'/0'/0'");

    const address = Deriver.deriveAddress('THT', 'main', xPub, 0, false);
  ...
  it('should be able to generate a valid BCH address', () => {
    ...
    const path = Deriver.pathFor('BCH', 'main');
    expect(path).to.equal("m/44'/145'/0'");

    const address = Deriver.deriveAddress('BCH', 'main', xPub, 0, false);
  ...
  it('should be able to generate a valid DOGE address', () => {
    ...
    const path = Deriver.pathFor('DOGE', 'main');
    expect(path).to.equal("m/44'/3'/0'");

    const address = Deriver.deriveAddress('DOGE', 'main', xPub, 0, false);
  ...
  it('should be able to generate a valid LTC address', () => {
    ...
    const path = Deriver.pathFor('LTC', 'main');
    expect(path).to.equal("m/44'/2'/0'");

    const address = Deriver.deriveAddress('LTC', 'main', xPub, 0, false);
  ...
  it('should be able to generate a valid ETH address', () => {
    ...
    const path = Deriver.pathFor('ETH', 'main');
    expect(path).to.equal("m/44'/60'/0'");

    const address = Deriver.deriveAddress('ETH', 'main', xPub, 0, false);
  ...
  it('should be able to generate a valid XRP address', () => {
    ...
    const path = Deriver.pathFor('XRP', 'main');
    expect(path).to.equal("m/44'/144'/0'");

    const address = Deriver.deriveAddress('XRP', 'main', xPub, 0, false);
  ...
  it('should be able to generate a valid MATIC address', () => {
    ...
    const path = Deriver.pathFor('MATIC', 'main');
    expect(path).to.equal("m/44'/60'/0'");

    const address = Deriver.deriveAddress('MATIC', 'main', xPub, 0, false);
    const expectedAddress = '0x9dbfE221A6EEa27a0e2f52961B339e95426931F9';
    expect(address).to.equal(expectedAddress);
  });
  ...
  it('should be able to generate a valid ETH address, privKey, pubKey', () => {
    ...
    const result = Deriver.derivePrivateKey('ETH', 'main', privKey, 0, false);
  ...
  it('should be able to generate a valid XRP address, privKey, pubKey', () => {
    ...
    const result = Deriver.derivePrivateKey('XRP', 'main', privKey, 0, false);
    const expectedResult = {
  ...
  it('should be able to get a valid XRP address from pubKey', () => {
    ...
    const result = Deriver.getAddress('XRP', 'main', pubKey, 'asd');
  ...
  it('should be able to generate a valid MATIC address, privKey, pubKey', () => {
    ...
    const result = Deriver.derivePrivateKey('MATIC', 'main', privKey, 0, false);
    const expectedResult = {
      address: '0xb497281830dE4F19a3482AbF3D5C35c514e6fB36',
      privKey: '62b8311c71f355c5c07f6bffe9b1ae60aa20d90e2e2ec93ec11b6014b2ae6340',
      pubKey: '0386d153aad9395924631dbc78fa560107123a759eaa3e105958248c60cd4472ad'
