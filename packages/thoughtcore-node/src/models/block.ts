...
import { ThoughtBlockType, ThoughtHeaderObj } from '../types/namespaces/Thought';
...
@LoggifyClass
export class ThoughtBlock extends BaseBlock<IThtBlock> {
  constructor(storage?: StorageService) {
    super(storage);
  }

  async addBlock(params: {
    block: ThoughtBlockType;
    parentChain?: string;
    forkHeight?: number;
  ...
  async getBlockOp(params: { block: ThoughtBlockType; chain: string; network: string }) {
    const { block, chain, network } = params;
    const header = block.header.toObject();
    ...
    const convertedBlock: IThtBlock = {
      chain,
      network,
      hash: block.hash,
      height,
  ...
  async handleReorg(params: { header?: ThoughtHeaderObj; chain: string; network: string }): Promise<boolean> {
  ...
  _apiTransform(block: Partial<MongoBound<IThtBlock>>, options?: TransformOptions): any {
    const transform = {
      _id: block._id,
      chain: block.chain,
...
export let ThoughtBlockStorage = new ThoughtBlock();
