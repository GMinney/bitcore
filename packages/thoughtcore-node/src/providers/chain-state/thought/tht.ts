...
export class THTStateProvider extends InternalStateProvider {
  constructor(chain: string = 'THT') {
    super(chain);
  }
  ...
  async getFee(params: GetEstimateSmartFeeParams) {
    ...
    return CacheStorage.getGlobalOrRefresh(
      ...
      async () => {
        ...
        // NOTE: rpcEstimate is in THT per kilobyte, estimates is in sats per byte
        ...
        feerate = Number((feerate / 1e5).toFixed(8)); // convert to THT per KB
