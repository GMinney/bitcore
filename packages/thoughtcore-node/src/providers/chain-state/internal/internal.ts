...
import { ThoughtBlockStorage, IThtBlock } from '../../../models/block';
...
export class InternalStateProvider implements IChainStateService {
  ...
  async getBlocks(params: GetBlockParams): Promise<Array<IBlock>> {
    const { query, options } = this.getBlocksQuery(params);
    let cursor = ThoughtBlockStorage.collection.find(query, options).addCursorFlag('noCursorTimeout', true);
    if (options.sort) {
    ...
    const blockTransform = (b: IThtBlock) => {
      let confirmations = 0;
      if (b.height && b.height >= 0) {
        confirmations = tipHeight - b.height + 1;
      }
      const convertedBlock = ThoughtBlockStorage._apiTransform(b, { object: true }) as IThtBlock;
  ...
  async streamMissingWalletAddresses(params: StreamWalletMissingAddressesParams) {
    ...
    const missingStream = cursor.stream().pipe(
      through2(
        { objectMode: true },
  ...
  async streamWalletTransactions(params: StreamWalletTransactionsParams) {
    ...
    transactionStream.stream().pipe(listTransactionsStream).pipe(res);
  }
  ...
  async getDailyTransactions(params: DailyTransactionsParams) {
    ...
    const results = await ThoughtBlockStorage.collection
      .aggregate<{
        date: string;
  ...
  async getLocalTip({ chain, network }) {
    return ThoughtBlockStorage.getLocalTip({ chain, network });
  }
  ...
  private extractAddress(address: string): string {
    const extractedAddress = address.replace(/^(thoughtcash:|bchtest:|thought:)/i, '').replace(/\?.*/, '');
