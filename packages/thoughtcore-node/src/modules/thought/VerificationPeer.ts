...
import { ThoughtBlockStorage } from '../../models/block';
...
export class VerificationPeer extends ThoughtP2PWorker implements IVerificationPeer {
  prevBlockNum = 0;
  prevHash = '';
  nextBlockHash = '';
  deepScan = false;
  ...
  async getBlockForNumber(currentHeight: number) {
    ...
    const [{ hash }] = await ThoughtBlockStorage.collection
    ...
    while (currentHeight < end) {
      ...
      for (let header of headers) {
        if (currentHeight <= end) {
          ...
          await ThoughtBlockStorage.processBlock({ chain, network, block, initialSyncComplete: true });
          const nextBlock = await ThoughtBlockStorage.collection.findOne({
            chain,
            network,
  ...
  async validateDataForBlock(blockNum: number, tipHeight: number, log = false) {
    ...
    const [block, blockTxs] = await Promise.all([
      ThoughtBlockStorage.collection.findOne({
        chain,
        network,
        height: blockNum,
        processed: true
    ...
    const [coinsForTx, mempoolTxs, blocksForHash, blocksForHeight, p2pBlock] = await Promise.all([
      ...
      ThoughtBlockStorage.collection.countDocuments({ chain, network, hash: firstHash }),
      ThoughtBlockStorage.collection.countDocuments({
        chain,
        network,
        height: blockNum,
        processed: true
