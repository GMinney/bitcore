...
import { ThtRpcTransactionStorage } from './transaction';

@LoggifyClass
export class ThtRpcBlockModel extends BaseBlock<IBlock> {
  ...
  async addBlock(params: {
    block: IThtRpcBlock;
    transactions: IThtRpcTransaction[];
    coins: IThtRpcCoin[];
    parentChain?: string;
    forkHeight?: number;
  ...
  async getBlockOp(params: { block: IThtRpcBlock; chain: string; network: string }) {
    const { block, chain, network } = params;
  ...
  async handleReorg(params: { block: IThtRpcBlock; chain: string; network: string }): Promise<boolean> {
    const { block, chain, network } = params;
    const prevHash = block.previousBlockHash;
    ...
    const reorgOps = [
      ...
      ThtRpcTransactionStorage.collection.deleteMany({ chain, network, blockHeight: { $gte: localTip.height } })
    ];
  ...
  _apiTransform(block: Partial<MongoBound<IThtRpcBlock>>, options?: TransformOptions): any {
    const transform = {
      _id: block._id,
      chain: block.chain,
...
export let ThtRpcBlockStorage = new ThtRpcBlockModel();
