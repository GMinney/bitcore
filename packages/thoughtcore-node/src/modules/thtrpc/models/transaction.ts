...
import { IThtRpcCoin, IThtRpcTransaction, ThtRpcTransactionJSON } from '../types';

@LoggifyClass
export class ThtRpcTransactionModel extends BaseTransaction<IThtRpcTransaction> {
  ...
  async batchImport(params: {
    txs: Array<IThtRpcTransaction>;
    coins: Array<IThtRpcCoin>;
    height?: number;
    mempoolTime?: Date;
    blockTime?: Date;
    blockHash?: string;
    ...
    if (params.height != undefined && params.height < SpentHeightIndicators.minimum) {
      for (let op of txOps) {
        ...
        const tx = { ...op.updateOne.update.$set, ...filter } as IThtRpcTransaction;
        await EventStorage.signalTx(tx);
      }

      for (const coinOp of coinOps) {
        const coin = { ...coinOp.updateOne.filter, ...coinOp.updateOne.update.$set } as IThtRpcCoin;
  ...
  async addTransactions(params: {
    ...
    if (parentChain && forkHeight && height != undefined && height < forkHeight) {
      const parentTxs = await ThtRpcTransactionStorage.collection
      ...
      return Promise.all(
        params.txs.map(async (tx: IThtRpcTransaction) => {
  ...
  async addCoins(params: {
    ...
    if (parentChain && forkHeight && height != undefined && height < forkHeight) {
      const parentChainCoins = await CoinStorage.collection
        .find({ blockHeight: height, chain: parentChain, network })
        .toArray();
      return parentChainCoins.map((coin: IThtRpcCoin) => {
        const { mintTxid } = coin;
      ...
      return params.coins.map((coin: IThtRpcCoin) => {
  ...
  getTransactions(params: { query: any; options: StreamingFindOptions<IThtRpcTransaction> }) {
  ...
  _apiTransform(
    tx: IThtRpcTransaction | Partial<MongoBound<IThtRpcTransaction>>,
    options?: TransformOptions
  ): ThtRpcTransactionJSON | string {
    const transaction: ThtRpcTransactionJSON = {
      txid: tx.txid || '',
      network: tx.network || '',
      chain: tx.chain || '',
      ...
      currency: valueOrDefault(tx.currency, 'THTRPC'),
...
export let ThtRpcTransactionStorage = new ThtRpcTransactionModel();
