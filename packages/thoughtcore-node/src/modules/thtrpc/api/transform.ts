...
export class ThoughtRpcWalletTransactions extends Transform {
  walletAddresses?: Array<IWalletAddress>;
  constructor(private wallet: IWallet, private csp: ThoughtRpcStateProvider) {
          ...
          if (!receiving) {
            this.push(
              JSON.stringify({
                id: tx.hash,
                txid: tx.hash,
                fee: Number(transaction.fee) * 1e6,
                size: 0,
                category: 'send',
                notions: -1 * Number(output.value) * 1e6,
          ...
          if (transaction.fee > 0) {
            this.push(
              JSON.stringify({
                ...
                txid: tx.hash,
                category: 'fee',
                notions: -1 * Number(transaction.fee) * 1e6,
                height: transaction.blockHeight,
                blockTime: transaction.blockTimeNormalized
              }) + '\n'
            );
          ...
          if (receiving) {
            this.push(
              JSON.stringify({
                ...
                size: 0,
                category: 'receive',
                notions: Number(output.value) * 1e6,
                height: transaction.blockHeight,
                address,
                outputIndex: changed.indexOf(address),
                blockTime: transaction.blockTimeNormalized
