...
import { AccountTransaction, BlockTransaction, IThtRpcTransaction, RpcTransaction } from '../types';
import { ThoughtRpcDbWalletTransactions } from './wallet-tx-transform';


export class ThoughtRpcStateProvider extends InternalStateProvider implements IChainStateService {
  config: any;
  static clients: { [network: string]: typeof CryptoRpc } = {};

  constructor(public chain: string = 'THTRPC') {
    super(chain, ThoughtRpcDbWalletTransactions);
    this.config = Config.chains[this.chain];
  }

  async getClient(network: string) {
    if (!ThoughtRpcStateProvider.clients[network]) {
      const networkConfig = this.config[network];
      const provider = networkConfig.provider;
      ThoughtRpcStateProvider.clients[network] = new CryptoRpc({
          chain: this.chain,
          host: provider.host,
          rpcPort: provider.port,
          protocol: provider.protocol 
        }).get(this.chain);
      await ThoughtRpcStateProvider.clients[network].rpc.connect();
    }

    try {
      if (ThoughtRpcStateProvider.clients[network].rpc.isConnected()) {
        await ThoughtRpcStateProvider.clients[network].getBlock();
      } else {
        await ThoughtRpcStateProvider.clients[network].rpc.connect();
      }
    } catch (e) {
    ...
    return ThoughtRpcStateProvider.clients[network];
  }
    ...
    return CacheStorage.getGlobalOrRefresh(
      ...
      async () => {
        ...
        try {
          ...
          logger.error(`Error getting THTRPC balance for ${address} on ${network}: ${JSON.stringify(e.data) || e.stack || e.message || e}`);
          throw e;
        }
      },
  ...
  async getBlockBeforeTime(params: GetBlockBeforeTimeParams) {
    ...
    const [block] = await ThtRpcBlockStorage.collection
      .find({
        chain,
        network,
  ...
  async getDataHostLedger(index, network) {
    ...
    const url = this.config[network].provider.dataHost;
    const jsonOpts: GotJSONOptions = { body: reqBody, json: true};
    const ledger = await got.post(url, jsonOpts);
  ...
  async getLocalTip(params: ChainNetwork) {
    return ThtRpcBlockStorage.getLocalTip(params);
  }
  ...
  transform(tx: BlockTransaction | RpcTransaction, network: string, block?: IBlock): IThtRpcTransaction {
    const date = block?.time ? new Date(block?.time) : this.getDateFromThoughtRpcTime((tx as RpcTransaction).date) || '';
    ...
    return {
      ...
      wallets: []
    } as IThtRpcTransaction;
  }

  transformAccountTx(tx: AccountTransaction, network: string): IThtRpcTransaction {
    const date = this.getDateFromThoughtRpcTime(tx.tx?.date) || '';
  ...
  async tag(
    chain: string,
    network: string,
    tx: IThtRpcTransaction,
    outputs: Array<ICoin> | any
  ): Promise<{ transaction: IThtRpcTransaction; coins: Array<ICoin> }> {
  ...
  getDateFromThoughtRpcTime(thoughtRpcTime?: number) {
    if (thoughtRpcTime == null) {
      return null;
    }
    // the thoughtRpc epoch is 2000-01-01
    return new Date(new Date('2000-01-01').getTime() + thoughtRpcTime * 1000);
  
  }
}

export const THTRPC = new ThoughtRpcStateProvider();
