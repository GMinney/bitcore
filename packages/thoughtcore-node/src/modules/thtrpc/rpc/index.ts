...
import { ThtRpcBlockModel, ThtRpcBlockStorage } from '../models/block';
import { ThtRpcTransactionStorage } from '../models/transaction';
import { IThtRpcBlock, IThtRpcCoin, IThtRpcTransaction } from '../types';


@LoggifyClass
export class ThtRpcP2pWorker extends BaseP2PWorker<any> {
  protected chainConfig: any;
  protected syncing: boolean;
  protected initialSyncComplete: boolean;
  public provider: ThoughtRpcStateProvider;
  ...
  public client?: typeof THTRPC;
  public blockModel: ThtRpcBlockModel;

  constructor({ chain, network, chainConfig, blockModel = ThtRpcBlockStorage }) {
    super({ chain, network, chainConfig, blockModel });
    this.blockModel = blockModel;
    this.chain = chain || 'THTRPC';
    this.network = network;
    this.chainConfig = chainConfig;
    this.syncing = false;
    this.initialSyncComplete = false;
    this.provider = new ThoughtRpcStateProvider(this.chain);
          ...
          .pipe(
            new Transform({
              ...
              transform: async (data, _, cb) => {
                ...
                const [lastTx] = await ThtRpcTransactionStorage.collection
                ...
                await ThtRpcTransactionStorage.batchImport({
                  txs: blockTxs,
                  coins: blockCoins,
                  chain,
                  network,
  ...
  async syncBlocks() {
    ...
    logger.info(`Starting ThtRpc Sync @ ${configuredStart}`);
    ourBestBlock = { height: configuredStart } as IThtRpcBlock;
