...
const { ThoughtBlockStorage } = require('../build/src/models/block');
...
Storage.start()
  .then(async () => {
    ...
    const cnt3 = await ThoughtBlockStorage.collection.countDocuments({ chain, network: oldNetwork });
    ...
    let blocks = await ThoughtBlockStorage.collection.find({ chain, network: oldNetwork }).project({ _id: 1 }).limit(batchSize).toArray();
    let blocksUpdated = 0;
    while (blocks.length > 0 && !quit) {
      const res = await ThoughtBlockStorage.collection.updateMany({ _id: { $in: blocks.map(c => c._id) } }, { $set: { network: newNetwork } });
      blocksUpdated += res.modifiedCount;
      console.log(`Updated ${blocksUpdated} blocks (${(blocksUpdated / cnt3 * 100).toFixed(2)}%)`);
      await wait(250);
      blocks = await ThoughtBlockStorage.collection.find({ chain, network: oldNetwork }).project({ _id: 1 }).limit(batchSize).toArray()
    }
