...
import { MintOp, SpendOp, TaggedThoughtTx, TransactionStorage, TxOp } from '../../../src/models/transaction';
import { WalletAddressStorage } from '../../../src/models/walletAddress';
import { ThoughtTransaction, TransactionInput } from '../../../src/types/namespaces/Thought';
...
describe('Transaction Model', function() {
  ...
  describe('Wallet Tagging', async () => {
    const tx = thoughtcoreLib.Transaction(TransactionFixture.transaction) as TaggedThoughtTx;
    const CURRENT_HEIGHT = 8534;
    ...
    it('should tag the transaction ops, and calculate the fee', async () => {
      function getCoinForInput(i: TransactionInput) {
        ...
        const inputTx = new thoughtcoreLib.Transaction(fixtureInput) as ThoughtTransaction;
        const coin = { spentTxid: tx.hash, value: inputTx.outputs[input.outputIndex].notions, wallets: [] };
        return coin;
      }
