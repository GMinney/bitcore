...
import { IThtBlock } from '../../../src/models/block';
...
describe('Block Model', function () {
  ...
  describe('addBlock', () => {
    ...
    it('should be able to add a block', async () => {
      let newBlock = Object.assign({ save: () => Promise.resolve() }, ThoughtBlockStorage, addBlockParams);

      mockStorage(newBlock);
      sandbox.stub(ThoughtBlockStorage, 'handleReorg').resolves();
      sandbox.stub(TransactionStorage, 'batchImport').resolves();

      const result = await ThoughtBlockStorage.addBlock(addBlockParams);
      expect(result);
    });
  });
  ...
  describe('BlockModel find options', () => {
    it('should be able to create query options', () => {
      ...
      const { query, options } = Storage.getFindOptions<MongoBound<IThtBlock>>(ThoughtBlockStorage, {
        since: id,
        paging: '_id',
        limit: 100,
        direction: -1
  ...
  describe('getLocalTip', () => {
    ...
    it('should return the new tip', async () => {
      let newBlock = Object.assign({ save: () => Promise.resolve() }, ThoughtBlockStorage, addBlockParams);
      mockStorage(newBlock);
      const params = { chain: 'THT', network: 'regtest' };
  ...
  describe('handleReorg', () => {
    ...
    it('should return if localTip hash equals the previous hash', async () => {
      Object.assign(ThoughtBlockStorage.collection, mockCollection(null));
      Object.assign(TransactionStorage.collection, mockCollection(null));
      Object.assign(CoinStorage.collection, mockCollection(null));
      let blockModelRemoveSpy = ThoughtBlockStorage.collection.deleteMany as sinon.SinonSpy;
      ...
      await ThoughtBlockStorage.handleReorg(params);
    ...
    it('should return if localTip height is zero', async () => {
      ...
      let params = Object.assign(ThoughtBlockStorage, blockMethodParams);
    ...
    it('should call blockModel deleteMany', async () => {
      ...
      let params = Object.assign(ThoughtBlockStorage, blockMethodParams);
      const removeSpy = ThoughtBlockStorage.collection.deleteMany as sinon.SinonSpy;
  ...
  describe('_apiTransform', () => {
    it('should return the transform object with block values', () => {
      ...
      const result = ThoughtBlockStorage._apiTransform(block, { object: true });
