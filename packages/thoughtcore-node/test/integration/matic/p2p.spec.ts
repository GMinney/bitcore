import * as ThoughtcoreClient from 'thoughtcore-client';
...
const { StreamUtil } = ThoughtcoreClient;
const chain = 'MATIC';
...
async function getWallet() {
  let wallet: ThoughtcoreClient.Wallet;
  try {
    wallet = await ThoughtcoreClient.Wallet.loadWallet({ name, storageType });
    await wallet.register();
    await wallet.syncAddresses();
    return wallet;
  } catch (e) {
    console.log('Creating a new matic wallet');
    wallet = await ThoughtcoreClient.Wallet.create({
      name,
      chain,
      network,
      baseUrl,
