...
async function addTx(tx: IThtTransaction, outputs: ICoin[]) {
  await TransactionStorage.collection.insertOne(tx as IThtTransaction);
...
async function makeMempoolTxChain(chain: string, network: string, startingTxid: string, chainLength = 1) {
  ...
  for (let i = 1; i <= chainLength; i++) {
    ...
    await addTx(badMempoolTx as IThtTransaction, badMempoolOutputs as ICoin[]);
    allTxids.push(txid);
    txid = nextTxid;
...
describe('Coin Model', function () {
  ...
  it('should appropriately mark coins related to transactions that are in mempool, but no longer valid', async () => {
    ...
    await TransactionStorage.collection.insertOne(blockTx as IThtTransaction);
