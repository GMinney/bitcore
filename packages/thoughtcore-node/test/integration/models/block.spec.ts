...
async function insertBlocks() {
  await ThoughtBlockStorage.collection.insertOne({
    chain: 'THT',
    network: 'regtest',
    height: 5,
...
describe('Block Model', function () {
  ...
  describe('addBlock', () => {
    it('should add a block when incoming block references previous block hash', async () => {
      await insertBlocks();
      await ThoughtBlockStorage.addBlock({
        block: TEST_BLOCK,
        chain: 'THT',
        network: 'regtest',
      ...
      const blocks = await ThoughtBlockStorage.collection
        .find({ chain: 'THT', network: 'regtest' })
        .sort({ height: 1 })
        .toArray();
      expect(blocks.length).to.equal(5);
      const ownBlock = blocks[4];
      expect(ownBlock.chain).to.equal('THT');
      ...
      expect(transaction[0].chain).to.equal('THT');
  ...
  describe('handleReorg', () => {
    it("should not reorg if the incoming block's prevHash matches the block hash of the current highest block", async () => {
      ...
      await ThoughtBlockStorage.handleReorg({
        header: {
      ...
      const result = await ThoughtBlockStorage.collection.find({ chain: 'THT', network: 'regtest' }).toArray();
    ...
    it('should successfully handle reorg', async () => {
      ...
      const removedBlock = await ThoughtBlockStorage.collection
        .find({
          chain: 'THT',
          network: 'regtest',
          height: {
      ...
      expect(unspentCoins[0].chain).to.equal('THT');
    ...
    it('should detect a fault in the block hashes', async () => {
      ...
      await ThoughtBlockStorage.collection.updateOne(
        { chain, network, hash: badHash },
        { $set: { previousBlockHash: 'aaaaa' } }
      );
      const invalidChain = await ThoughtBlockStorage.validateLocatorHashes({ chain, network });
    ...
    it('should detect a missing block', async () => {
      ...
      await ThoughtBlockStorage.collection.deleteOne({ chain, network, hash: badHash });
      const invalidChain = await ThoughtBlockStorage.validateLocatorHashes({ chain, network });
