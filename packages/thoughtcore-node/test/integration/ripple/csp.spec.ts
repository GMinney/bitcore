...
describe('Ripple Api', function() {
  ...
  for (const tx of RippleTxs) {
    it('should transform a ripple rpc response into a thoughtcore transaction: ' + tx.hash, async () => {
      const thoughtcoreTx = await XRP.transform(tx, 'testnet');
      expect(thoughtcoreTx).to.have.property('chain');
      expect(tx.Account).to.eq(thoughtcoreTx.from);
      expect(tx.ledger_index).to.eq(thoughtcoreTx.blockHeight);
      expect(tx.Fee).to.eq((thoughtcoreTx.fee).toString());
      ...
      if (tx.meta.delivered_amount) {
        const modNodes = tx.meta.AffectedNodes.filter(n => 'ModifiedNode' in n && n.ModifiedNode.FinalFields?.Account === thoughtcoreTx.to);
        const createNodes = tx.meta.AffectedNodes.filter(n => 'CreatedNode' in n && n.CreatedNode.NewFields.Account === thoughtcoreTx.to);
        expect(modNodes.length + createNodes.length > 0).to.equal(true);
        expect(tx.meta.delivered_amount).to.eq(thoughtcoreTx.value.toString());
  ...
  it('should tag txs from a wallet', async () => {
    ...
    for (const tx of RippleTxs) {
      const thoughtcoreTx = (await XRP.transform(tx, network)) as IXrpTransaction;
      const thoughtcoreCoins = XRP.transformToCoins(tx, network);
      const { transaction, coins } = await XRP.tag(chain, network, thoughtcoreTx, thoughtcoreCoins);
  ...
  it('should save tagged transactions to the database', async () => {
    ...
    for (const tx of RippleTxs) {
      const thoughtcoreTx = XRP.transform(tx, network) as IXrpTransaction;
      const thoughtcoreCoins = XRP.transformToCoins(tx, network);
      const { transaction, coins } = await XRP.tag(chain, network, thoughtcoreTx, thoughtcoreCoins);
