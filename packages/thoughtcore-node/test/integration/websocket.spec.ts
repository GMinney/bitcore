...
const { PrivateKey } = require('thoughtcore-lib');
...
import { Client } from 'thoughtcore-client';
...
describe('Websockets', function () {
  ...
  beforeEach(async () => {
    ...
    await connected;
    p2pWorker = new ThoughtP2PWorker({
      chain,
      network,
      chainConfig
    });
  ...
  it('should get websocket events', async () => {
    socket.emit('room', '/THT/regtest/inv');
