...
import { ThoughtP2PWorker } from '../../src/modules/thought/p2p';
import { VerificationPeer } from '../../src/modules/thought/VerificationPeer';
...
function addBlock1() {
  return ThoughtBlockStorage.collection.insertOne({
    chain,
    network,
    height: 8976158,
...
describe('VerificationPeer', function () {
  ...
  it('should save any mempoool transactions', async () => {
    const chainConfig = Config.chainConfig({ chain, network });
    const worker = new ThoughtP2PWorker({ chain, network, chainConfig });
