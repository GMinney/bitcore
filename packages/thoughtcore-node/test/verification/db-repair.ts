...
(async () => {
  ...
  const handleRepair = async data => {
    try {
      const tip = await ThoughtBlockStorage.getLocalTip({ chain, network });
      switch (data.type) {
        ...
        case 'DUPE_BLOCKHASH':
          const dupeBlock = await ThoughtBlockStorage.collection
          ...
          if (dry) {
            ...
            await ThoughtBlockStorage.collection.deleteMany({
              chain,
              network,
