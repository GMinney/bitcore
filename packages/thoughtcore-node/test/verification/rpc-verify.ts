...
import { IThtTransaction, TransactionStorage } from '../../src/models/transaction';
...
export async function blocks(
  ...
  // check each block
  const cursor = ThoughtBlockStorage.collection.find({
    chain: info.chain,
    network: info.network
  });

  while (await cursor.hasNext()) {
    const block: IThtBlock | null = await cursor.next();
    if (!block) break;
...
export async function transactions(
  ...
  while (await txcursor.hasNext()) {
    const tx: IThtTransaction | null = await txcursor.next();
    if (!tx) {
      break;
    }
