...
const { Transaction, PrivateKey } = thoughtcoreLib;
const UnspentOutput = Transaction.UnspentOutput;

import config from '../../src/config';
import { Storage } from '../../src/services/storage';
import { ThoughtBlockStorage } from '../../src/models/block';
import { ThoughtBlockType } from '../../src/types/namespaces/Thought/Block';
...
function* generateBlocks(blockCount: number, blockSizeMb: number) {
  let prevBlock: ThoughtBlockType | undefined = undefined;
...
function preGenerateBlocks(blockCount: number, blockSizeMb: number) {
  const blocks = new Array<ThoughtBlockType>();
...
function generateBlock(blockSizeMb: number, previousBlock?: ThoughtBlockType): ThoughtBlockType {
  const txAmount = 100000;
  const prevHash = previousBlock ? previousBlock.hash : '';
  let block: ThoughtBlockType = {
    hash: randomHash(),
    transactions: [],
    toBuffer: () => {
...
async function benchmark(blockCount: number, blockSizeMb: number) {
  ...
  for (let block of blocks) {
    ...
    await ThoughtBlockStorage.addBlock({ block, chain: 'BENCH', network: 'MARK', initialSyncComplete: false });
  }
  process.stdout.write('\n');
