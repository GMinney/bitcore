# Thoughtcore Wallet Service

[![NPM Package](https://img.shields.io/npm/v/thoughtcore-wallet-service.svg?style=flat-square)](https://www.npmjs.org/package/thoughtcore-wallet-service)
[![Build Status](https://img.shields.io/travis/thoughtnetwork/thoughtcore-wallet-service.svg?branch=master&style=flat-square)](https://travis-ci.org/thoughtnetwork/thoughtcore-wallet-service)
[![Coverage Status](https://coveralls.io/repos/thoughtnetwork/thoughtcore-wallet-service/badge.svg?branch=master)](https://coveralls.io/r/thoughtnetwork/thoughtcore-wallet-service?branch=master)

**A Multisig HD Thoughtcore Wallet Service.**

## Description

Thoughtcore Wallet Service facilitates multisig HD wallets creation and operation through a (hopefully) simple and intuitive REST API.

BWS can usually be installed within minutes and accommodates all the needed infrastructure for peers in a multisig wallet to communicate and operate – with minimum server trust.

See [thoughtcore-wallet-client](https://github.com/thoughtnetwork/thoughtcore/tree/master/packages/thoughtcore-wallet-client) for the _official_ client library that communicates to BWS and verifies its response. Also check [thoughtcore-wallet](https://github.com/thoughtnetwork/thoughtcore/tree/master/packages/thoughtcore-wallet) for a simple CLI wallet implementation that relies on BWS.

BWS is been used in production enviroments for [Copay Wallet](https://copay.io), [Thoughtnetwork App wallet](https://thoughtnetwork.com/wallet) and others.

More about BWS at https://blog.thoughtnetwork.com/announcing-the-thoughtcore-wallet-suite/

## Getting Started

```sh
 git clone https://github.com/thoughtnetwork/thoughtcore-wallet-service.git
 cd thoughtcore-wallet-service
 npm install
 npm start
```
...
BWS supports SSL and Clustering. For a detailed guide on installing BWS with extra features see [Installing BWS](https://github.com/thoughtnetwork/thoughtcore/blob/master/packages/thoughtcore-wallet-service/installation.md).
...
```sh
  pm2 start app.js --name "thought-wallet-service"
```
...
- A copayer could join the wallet more than once, and there is no mechanism to prevent this. See [wallet](https://github.com/thoughtnetwork/thoughtcore/tree/master/packages/thoughtcore-wallet)'s confirm command, for a method for confirming copayers.
...
@dabura667 made a report about how to use letsencrypt with BWS: https://github.com/thoughtnetwork/thoughtcore-wallet-service/issues/423
...
**thoughtcore-wallet-service/bws.config.js**
...
Note: all currency amounts are in units of notions (1/100,000,000 of a thought).
...
See [Thoughtcore Wallet Client](https://github.com/thoughtnetwork/thoughtcore/tree/master/packages/thoughtcore-wallet-client) for implementation details.
...
- Wallet object. (see [fields on the source code](https://github.com/thoughtnetwork/thoughtcore/blob/master/packages/thoughtcore-wallet-service/src/lib/model/wallet.ts)).
...
- List of pending TX Proposals. (see [fields on the source code](https://github.com/thoughtnetwork/thoughtcore/blob/master/packages/thoughtcore-wallet-service/src/lib/model/txproposal.ts))
...
- List of Addresses object: (https://github.com/thoughtnetwork/thoughtcore/blob/master/packages/thoughtcore-wallet-service/src/lib/model/address.ts). This call is mainly provided so the client check this addresses for incoming transactions (using a service like [Insight](https://insight.thoughtcore.io)
...
- toAddress: RCPT Thought address.
- amount: amount (in notions) of the mount proposed to be transfered
...
- TX Proposal object. (see [fields on the source code]https://github.com/thoughtnetwork/thoughtcore/blob/master/packages/thoughtcore-wallet-service/src/lib/model/txproposal.ts)). `.id` is probably needed in this case.

### `/v2/txproposals/:id/publish`: Publish the previously created `temporary` tx proposal

Returns:

- TX Proposal object. (see [fields on the source code](https://github.com/thoughtnetwork/thoughtcore/blob/master/packages/thoughtcore-wallet-service/src/lib/model/txproposal.ts)).

### `/v3/addresses/`: Request a new main address from wallet . (creates an address on normal conditions)

Returns:

- Address object: (https://github.com/thoughtnetwork/thoughtcore/blob/master/packages/thoughtcore-wallet-service/src/lib/model/address.ts). Note that `path` is returned so client can derive the address independently and check server's response.
...
- TX Proposal object. (see [fields on the source code](https://github.com/thoughtnetwork/thoughtcore/blob/master/packages/thoughtcore-wallet-service/src/lib/model/txproposal.ts)). `.status` is probably needed in this case.
