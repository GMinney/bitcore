...
export class V8 {
  ...
  _getAuthClient(wallet) {
    ...
    return new this.Client({
      baseUrl: this.baseUrl,
      authKey: Thoughtcore_[this.chain].PrivateKey(wallet.beAuthPrivateKey2)
    });
  }
  ...
  /**
   * Broadcast a transaction to the thought network
   */
  ...
  initSocket(callbacks) {
    ...
    const getAuthPayload = host => {
      ...
      const authKeyObj = new Thoughtcore.PrivateKey(authKey);
