...
export class Client {
  ...
  sign(params: { method: string; url: string; payload?: any }) {
    ...
    const messageHash = thoughtcoreLib.crypto.Hash.sha256sha256(Buffer.from(message));

    // TODO: Should use thoughtcore-lib instead of an external dependency. Will want to add tests.
    // const privateKey = thoughtcoreLib.PrivateKey.fromBuffer(this.authKey.bn.toBuffer({ size: 32 }));
    // const sig = thoughtcoreLib.crypto.ECDSA.sign(messageHash, privateKey);
