...
export class FiatRateService {
  ...
  _retrieve(provider, coin, cb) {
    ...
    const ts = Date.now();
    if (Constants.THOUGHTNETWORK_USD_STABLECOINS[coinUC]) {
      return this.getRatesForStablecoin({ code: 'USD', ts }, handleCoinsRates);
    }

    if (Constants.THOUGHTNETWORK_EUR_STABLECOINS[coinUC]) {
  ...
  getRate(opts, cb) {
    ...
    async.map(
      ...
      (ts, cb) => {
        if (coin === 'wtht') {
          logger.info('Using tht for wtht rate.');
          coin = 'tht';
        }
        if (coin === 'weth') {
  ...
  getRates(opts, cb) {
    ...
    async.map(
      Object.values(Constants.THOUGHTNETWORK_SUPPORTED_COINS),
      (coin, cb) => {
        rates[coin] = [];
        async.map(
          currencies,
  ...
  getRatesByCoin(opts, cb) {
    ...
    if (Constants.THOUGHTNETWORK_USD_STABLECOINS[coin.toUpperCase()]) {
      return this.getRatesForStablecoin({ code: 'USD', ts }, cb);
    }

    if (Constants.THOUGHTNETWORK_EUR_STABLECOINS[coin.toUpperCase()]) {
  ...
  getHistoricalRates(opts, cb) {
    ...
    const coins = ['tht', 'bch', 'eth', 'matic', 'xrp', 'doge', 'ltc', 'shib', 'ape'];
   ...
   * @param {String} [opts.coin] crypto to base the fiat rates on (defaults to 'tht')
