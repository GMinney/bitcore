...
export interface IChain {
  ...
  getThoughtcoreTx(txp: TxProposal, opts: { signed: boolean });
...
class ChainProxy {
  ...
  getChain(coin: string): string {
    try {
      ...
      if (
        Constants.THOUGHTNETWORK_SUPPORTED_ETH_ERC20[normalizedChain.toUpperCase()] ||
      ...
      return Defaults.CHAIN; // coin should always exist but most unit test don't have it -> return tht as default
    }
  }
  ...
  getThoughtcoreTx(txp: TxProposal, opts = { signed: true }) {
    return this.get(txp.chain).getThoughtcoreTx(txp, { signed: opts.signed });
  }
  ...
  addSignaturesToThoughtcoreTx(chain, tx, inputs, inputPaths, signatures, xpub, signingMethod) {
    this.get(chain).addSignaturesToThoughtcoreTx(tx, inputs, inputPaths, signatures, xpub, signingMethod);
  }
