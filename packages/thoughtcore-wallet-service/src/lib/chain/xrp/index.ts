...
export class XrpChain implements IChain {
   ...
   */
  private convertThoughtcoreBalance(thoughtcoreBalance, locked, reserve = Defaults.MIN_XRP_BALANCE) {
    ...
    bc.getBalance(wallet, (err, balance) => {
      ...
      bc.getReserve((err, reserve) => {
        ...
        server.getPendingTxs(opts, (err, txps) => {
          ...
          const convertedBalance = this.convertThoughtcoreBalance(balance, lockedSum, reserve);
