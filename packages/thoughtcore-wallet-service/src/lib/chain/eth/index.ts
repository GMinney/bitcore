...
export class EthChain implements IChain {
  ...
  /**
   * Converts Thoughtcore Balance Response.
   * @param {Object} thoughtcoreBalance - { unconfirmed, confirmed, balance }
   * @param {Number} locked - Sum of txp.amount
   * @returns {Object} balance - Total amount & locked amount.
   */
  private convertThoughtcoreBalance(thoughtcoreBalance, locked) {
    const { unconfirmed, confirmed, balance } = thoughtcoreBalance;
        ...
        if (opts.multiSendContractAddress) {
          try {
            const data = this.encodeContractParameters(
              Constants.THOUGHTNETWORK_CONTRACTS.MULTISEND,
  ...
  getThoughtcoreTx(txp, opts = { signed: true }) {
    ...
    let tx = {
      ...
      toObject: () => {
        let ret = _.clone(txp);
        ret.outputs[0].notions = ret.outputs[0].amount;
        return ret;
      },
      getFee: () => {
        return txp.fee;
    ...
    if (opts.signed) {
      ...
      sigs.forEach(x => {
        this.addSignaturesToThoughtcoreTx(tx, txp.inputs, txp.inputPaths, x.signatures, x.xpub);
      });
    }
  ...
  encodeContractParameters(contract, params, opts) {
    if (contract === Constants.THOUGHTNETWORK_CONTRACTS.MULTISEND) {
      const web3 = new Web3();
      return {
        ...
        type: Constants.THOUGHTNETWORK_CONTRACTS.MULTISEND
      };
    }
  }
  ...
  checkTx(txp) {
    try {
      const tx = this.getThoughtcoreTx(txp);
    } catch (ex) {
      logger.debug('Error building Thoughtcore transaction: %o', ex);
      return ex;
    }
  ...
  addSignaturesToThoughtcoreTx(tx, inputs, inputPaths, signatures, xpub) {
