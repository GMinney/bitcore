...
export class PushNotificationsService {
  ...
  start(opts, cb) {
    ...
    this.defaultUnit = opts.pushNotificationsOpts.defaultUnit || 'tht';
            ...
            (subs, next) => {
              const notifications = _.map(subs, sub => {
                ...
                if (sub.externalUserId) {
                  ...
                  const custom_uri = `thoughtnetwork://wallet?walletId=${walletId}&tokenAddress=${tokenAddress}&multisigContractAddress=${multisigContractAddress}&copayerId=${copayerId}&coin=${coin}&chain=${chain}&network=${network}&notification_type=${notification_type}&title=${title}&body=${body}`;
                  notificationData = {
        ...
        if (data.tokenAddress) {
          ...
          if (Constants.ETH_TOKEN_OPTS[tokenAddress]) {
            ...
            if (customTokensData && customTokensData[tokenAddress]) {
              ...
              opts.toNotions = 10 ** customTokensData[tokenAddress].decimals;
              opts.decimals = {
                maxDecimals: 6,
                minDecimals: 2
              };
