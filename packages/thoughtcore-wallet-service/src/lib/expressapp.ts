  ...
  start(opts, cb) {
    ...
    this.app.use((req, res, next) => {
      if (config.maintenanceOpts.maintenanceMode === true) {
        // send a 503 error, with a message to the thoughtnetwork status page
        let errorCode = 503;
    ...
    router.get('/latest-version', async (req, res) => {
      ...
      try {
        ...
        var options = {
          uri: 'https://api.github.com/repos/thoughtnetwork/wallet/releases/latest',
          headers: {
            'User-Agent': 'Copay'
          },
          json: true
