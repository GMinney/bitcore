...
/**
 * Creates an instance of the Thoughtcore Wallet Service.
 * @constructor
 */
export class WalletService implements IWalletService {
   ...
   * @param {string} opts.coin[='tht'] - The coin for this wallet (tht, bch, eth, doge, ltc).
   * @param {string} opts.chain[='tht'] - The chain for this wallet (tht, bch, eth, doge, ltc).
   * @param {string} opts.network[='livenet'] - The Thought network for this wallet.
  ...
  createWallet(opts, cb) {
    ...
    try {
      pubKey = new Thoughtcore.PublicKey.fromString(opts.pubKey);
    } catch (ex) {
  ...
  _verifyRequestPubKey(requestPubKey, signature, xPubKey) {
    const pub = new Thoughtcore.HDPublicKey(xPubKey).deriveChild(Constants.PATHS.REQUEST_KEY_AUTH).publicKey;
   ...
   * @param {string} opts.coin[='tht'] - The expected coin for this wallet (tht, bch, eth, doge, ltc).
   * @param {string} opts.chain[='tht'] - The expected chain for this wallet (tht, bch, eth, doge, ltc).
  ...
  joinWallet(opts, cb) {
    ...
    if (!opts.hardwareSourcePublicKey) {
      ...
      try {
        xPubKey = Thoughtcore_[opts.chain].HDPublicKey(opts.xPubKey);
      } catch (ex) {
   ...
   * @param {string} opts.unit - Thought unit used to format amounts in notifications.
  ...
  savePreferences(opts, cb) {
    ...
    const preferences = [
      ...
      },
      {
        name: 'unit',
        isValid(value) {
          return _.isString(value) && _.includes(['tht', 'bit'], value.toLowerCase());
        }
      },
      {
        name: 'tokenAddresses',
    ...
    async.series(
      [
        ...
        next => {
          ...
          const dustThreshold = Thoughtcore_[wallet.chain].Transaction.DUST_AMOUNT;
          ...
          bc.getUtxos(
            ...
            (err, utxos) => {
              ...
              allUtxos = utxos.filter(x => x.notions >= dustThreshold && !unusableAddresses.includes(x.address));
  ...
  getUtxos(opts, cb) {
    ...
    if (opts.addresses) {
      ...
      this.getWallet({}, (err, wallet) => {
        ...
        const A = Thoughtcore_[wallet.chain].Address;
   ...
   * @param {number} opts.feePerKb - Optional. Specify the fee per KB for this TX (in notions).
    ...
    bc.estimateFee(points, (err, result) => {
      ...
      const levels = _.fromPairs(
        _.map(points, p => {
          ...
          // NOTE: ONLY THT/BCH/DOGE/LTC expect feePerKb to be Thought amounts
          // others... expect wei.
   ...
   * @param {Object} opts
   * @param {string} [opts.coin = 'tht'] - The coin to estimate fee levels from.
   * @param {string} [opts.chain = 'tht'] - The coin to estimate fee levels from.
   * @param {string} [opts.network = 'livenet'] - The Thought network to estimate fee levels from.
   * @returns {Object} feeLevels - A list of fee levels & associated amount per kB in notions.
   */
      ...
      [
        ...
        next => {
          ...
          _.each(opts.outputs, x => {
            ...
            let newAddr;
            try {
              newAddr = Thoughtcore_['bch'].Address(x.toAddress).toLegacyAddress();
            } catch (e) {
              return next(e);
            }
   ...
   * @param {number} opts.outputs[].amount - Amount to transfer in notions.
   ...
   * @param {Boolean} opts.enableRBF - Optional. enable THT Replace By Fee
   ...
   * @param {Object} opts
   * @param {string} [opts.coin = 'tht'] - The coin for this transaction.
   * @param {string} [opts.chain = 'tht'] - The coin for this transaction.
   * @param {string} [opts.network = 'livenet'] - The Thought network for this transaction.
        ...
        async (err, txp) => {
          ...
          try {
            if (!txp.sign(this.copayerId, opts.signatures, copayer.xPubKey)) {
              ...
              const raw = ChainService.getThoughtcoreTx(txp).uncheckedSerialize();
  ...
  _normalizeTxHistory(walletId, txs: any[], dustThreshold, bcHeight, cb) {
    ...
    txs = _.filter(txs, tx => {
      ...
      if (tx.category == 'receive') {
        if (tx.notions < dustThreshold) return false;

        const output = {
          address: tx.address,
          amount: Math.abs(tx.notions)
        };
        ...
        _.map([].concat(txs), tx => {
          ...
          function recreateAbiType(effects) {
            ...
            if (effects && effects.length) {
              ...
              if (erc20Transfer) {
                // This is the only data used in old wallet and thoughtnetwork-app
          ...
          const ret = {
            ...
            fees: tx.fee || (indexedFee[tx.txid] ? Math.abs(indexedFee[tx.txid].notions) : null),
            time: t,
            size: tx.size,
            amount: 0,
            action: undefined,
          ...
          switch (tx.category) {
            case 'send':
              ret.action = 'sent';
              ret.amount = Math.abs(_.sumBy(tx.outputs, 'amount')) || Math.abs(tx.notions);
            ...
            case 'move':
              ret.action = 'moved';
              ret.amount = Math.abs(tx.notions);
    ...
    this.storage.getWalletAddressChecked(wallet.id, (err, checkedTotal) => {
      ...
      this.storage.walletCheck({ walletId: wallet.id }).then((localCheck: { sum: number }) => {
        bc.getCheckData(wallet, (err, serverCheck) => {
          ...
          if (err) {
            this.logw('Error at thoughtcore WalletCheck, ignoring' + err);
            return cb();
          }
   ...
   * @param {String} [opts.provider] - A provider of exchange rates (default 'Thought').
   ...
   * @param {Object} opts
   * @param {String} opts.coin - The coin requested (tht, bch, eth, xrp, , ltc).
   * @param {String} [opts.code] - Currency ISO code (e.g: USD, EUR, ARS).
   * @param {Date} [opts.ts] - A timestamp to base the rate on (default Date.now()).
   * @param {String} [opts.provider] - A provider of exchange rates (default 'Thought').
   * @returns {Array} rates - The exchange rate.
   ...
   * @param {string} opts.thoughtnetworkIdLocationCountry - (Optional) Country registered as address of the user logged in with ThoughtnetworkId.
   * @param {string} opts.thoughtnetworkIdLocationState - (Optional) State registered as address of the user logged in with ThoughtnetworkId.
  ...
  getServicesData(opts, cb) {
    ...
    const isLoggedIn = !!opts?.thoughtnetworkIdLocationCountry;

    // Swap crypto rules
    const swapUsaBannedStates = ['HI', 'LA', 'NY'];

    if (
      // Logged in with thoughtnetworkId
      (['US', 'USA'].includes(opts?.thoughtnetworkIdLocationCountry?.toUpperCase()) && swapUsaBannedStates.includes(opts?.thoughtnetworkIdLocationState?.toUpperCase())) ||
    ...
    if (
      // Logged in with thoughtnetworkId
      (['US', 'USA'].includes(opts?.thoughtnetworkIdLocationCountry?.toUpperCase()) && buyCryptoUsaBannedStates.includes(opts?.thoughtnetworkIdLocationState?.toUpperCase())) ||
      // Logged out (IP restriction)
    ...
    if (
      // Logged in with thoughtnetworkId
      (['US', 'USA'].includes(opts?.thoughtnetworkIdLocationCountry?.toUpperCase()) && sellCryptoUsaBannedStates.includes(opts?.thoughtnetworkIdLocationState?.toUpperCase())) ||
      // Logged out (IP restriction)
  ...
  moonpayGetSignedPaymentUrl(req): { urlWithSignature: string } {
    ...
    const URLSignatureHash: string = Thoughtcore.crypto.Hash.sha256hmac(
  ...
  wyreWalletOrderQuotation(req): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      const XApiSignatureHash: string = Thoughtcore.crypto.Hash.sha256hmac(
  ...
  changellySignRequests(message, secret: string) {
    ...
    const sign: string = Thoughtcore.crypto.Hash.sha512hmac(
