...
const { ThoughtcoreLib } = require('crypto-wallet-core');
const { ChainService } = require('../../ts_build/lib/chain');
const { ThtChain } = require('../../ts_build/lib/chain/tht');
...
describe('Chain THT', function () {

  describe('#getThoughtcoreTx', function () {
    it('should create a valid thoughtcore TX', function () {
    ...
    it('should order outputs as specified by outputOrder', function () {
      ...
      var t2 = ChainService.getThoughtcoreTx(txp);
      t2.getChangeOutput().should.deep.equal(t2.outputs[0]);
    });

    it('should create a valid signed thoughtcore TX', function () {
    ...
    it('should create a valid unsigned thoughtcore TX', function () {
      var txp = TxProposal.fromObj(signedTxp);
      var t = ChainService.getThoughtcoreTx(txp, { signed: false });
      should.exist(t);
  ...
  describe('#getEstimatedSize', function () {
    let tht, fromAddress, witnessFromAddress, simpleUtxoWith1THT, simpleWitnessUtxoWith1THT, changeAddress, toAddress, privateKey;
    ...
    before(function () {
      ...
      simpleUtxoWith1THT = {
        ...
        outputIndex: 0,
        script: ThoughtcoreLib.Script.buildPublicKeyHashOut(fromAddress).toString(),
        notions: 1e8,
      };

      simpleWitnessUtxoWith1THT = {
        address: witnessFromAddress,
        txId: '7e6b603779c8af58284566cf1b655395fffbefaf1c0a080d9aff43f0af05d873',
        outputIndex: 0,
        script: ThoughtcoreLib.Script.fromAddress(witnessFromAddress).toString(),
        notions: 1e8
      };
      const privKey = new ThoughtcoreLib.PrivateKey();
    ...
    it('1  input p2pkh,1 output p2pkh  ', function () {
      ...
      const estimatedLength = tht.getEstimatedSize(x);

      // Create a similar TX.
      let tx = new ThoughtcoreLib.Transaction();
    ...
    it('1  input p2pkh,  2 output p2pkh, 1 output p2sh  ', function () {
      ...
      let tx = new ThoughtcoreLib.Transaction();
      tx.from(simpleUtxoWith1THT)
        .to([{ address: toAddress, notions: 1e7 }, { address: toAddress, notions: 1e6 }])
    ...
    const p2shPrivateKey1 = ThoughtcoreLib.PrivateKey.fromWIF('cNuW8LX2oeQXfKKCGxajGvqwhCgBtacwTQqiCGHzzKfmpHGY4TE9');
    const p2shPublicKey1 = p2shPrivateKey1.toPublicKey();
    const p2shPrivateKey2 = ThoughtcoreLib.PrivateKey.fromWIF('cTtLHt4mv6zuJytSnM7Vd6NLxyNauYLMxD818sBC8PJ1UPiVTRSs');
    const p2shPublicKey2 = p2shPrivateKey2.toPublicKey();
    const p2shPrivateKey3 = ThoughtcoreLib.PrivateKey.fromWIF('cQFMZ5gP9CJtUZPc9X3yFae89qaiQLspnftyxxLGvVNvM6tS6mYY');
    const p2shPublicKey3 = p2shPrivateKey3.toPublicKey();

    const p2shAddress = ThoughtcoreLib.Address.createMultisig([
      p2shPublicKey1,
      p2shPublicKey2,
      //      p2shPublicKey3
    ], 2, 'testnet3');
    const p2shUtxoWith1THT = {
      address: p2shAddress.toString(),
      txId: 'a477af6b2667c29670467e4e0728b685ee07b240235771862318e29ddbe58458',
      outputIndex: 0,
      script: ThoughtcoreLib.Script(p2shAddress).toString(),
      notions: 1e8
    };

    const p2wshAddress = ThoughtcoreLib.Address.createMultisig([
      p2shPublicKey1,
      p2shPublicKey2,
      //      p2shPublicKey3
    ], 2, 'testnet3', null, ThoughtcoreLib.Address.PayToWitnessScriptHash);
    const p2wshUtxoWith1THT = {
      address: p2wshAddress.toString(),
      txId: 'a477af6b2667c29670467e4e0728b685ee07b240235771862318e29ddbe58458',
      outputIndex: 0,
      script: ThoughtcoreLib.Script(p2wshAddress).toString(),
      notions: 1e8
    };
    ...
    it('1  input p2sh, 2 P2PKH outputs:  ', function () {
      ...
      // Create a similar TX.
      let tx = new ThoughtcoreLib.Transaction();
      tx.from(p2shUtxoWith1THT, [p2shPublicKey1, p2shPublicKey2], 2)
        .to([{ address: toAddress, notions: 1e7 }, { address: toAddress, notions: 1e6 }])
        .change(changeAddress)
