...
const { ThoughtcoreLibLtc } = require('crypto-wallet-core');
...
describe('Chain LTC', () => {
  describe('#getThoughtcoreTx', () => {
    it('should create a valid thoughtcore TX', () => {
      const txp = TxProposal.fromObj(aTXP());
      const t = ChainService.getThoughtcoreTx(txp);
      should.exist(t);
    });
    it('should order outputs as specified by outputOrder', () => {
      ...
      const t2 = ChainService.getThoughtcoreTx(txp);
      t2.getChangeOutput().should.deep.equal(t2.outputs[0]);
    });

    it('should create a valid signed thoughtcore TX', () => {
  ...
  describe('#getEstimatedSize', () => {
    ...
    before(() =>  {
      ...
      simpleUtxoWith1LTC = {
        ...
        outputIndex: 0,
        script: ThoughtcoreLibLtc.Script.buildPublicKeyHashOut(fromAddress).toString(),
        notions: 1e8,
      };

      const privKey = new ThoughtcoreLibLtc.PrivateKey();
    });
    ...
    it('1 input p2pkh,1 output p2pkh: Margin should be 10%', () => {
      ...
      // Create a similar TX.
      let tx = new ThoughtcoreLibLtc.Transaction();
    ...
    const p2shPrivateKey2 = ThoughtcoreLibLtc.PrivateKey.fromWIF('T4PcE9qqC9UNhB5Xb694epiXXEYW6xp6aA8QHT4UB83UfcDVicew');
    const p2shPublicKey2 = p2shPrivateKey2.toPublicKey();

    const p2shAddress = ThoughtcoreLibLtc.Address.createMultisig([
      p2shPublicKey1,
      p2shPublicKey2,
    ], 2, 'testnet');
    const p2shUtxoWith1THT = {
      ...
      txId: 'a477af6b2667c29670467e4e0728b685ee07b240235771862318e29ddbe58458',
      outputIndex: 0,
      script: ThoughtcoreLibLtc.Script(p2shAddress).toString(),
      notions: 1e8
    };
