  ...
  describe('#getEstimatedSize', () => {
    ...
    before(() => {
      ...
      simpleUtxo = {
        ...
        outputIndex: 0,
        script: ThoughtcoreLibDoge.Script.buildPublicKeyHashOut(fromAddress).toString(),
        notions: 1e9,
      };

      const privKey = new ThoughtcoreLibDoge.PrivateKey();
    });
    ...
    it('1 input p2pkh,1 output p2pkh: Margin should be 10%', () => {
      ...
      // Create a similar TX.
      let tx = new ThoughtcoreLibDoge.Transaction();
      tx.from(simpleUtxo)
    ...
    const p2shPrivateKey2 = ThoughtcoreLibDoge.PrivateKey.fromWIF('QQrWAEuSPk8TZF9rakoHGNiKmmhzshSEYkRp2J59TrhCXTkraP65');
    const p2shPublicKey2 = p2shPrivateKey2.toPublicKey();

    const p2shAddress = ThoughtcoreLibDoge.Address.createMultisig([
      p2shPublicKey1,
      p2shPublicKey2,
    ], 2, 'testnet');
    const p2shUtxoWithDOGE = {
      ...
      txId: '0820f0d4aafe9f142d18c6400ed3d58af5f0834b4b05ae41b3adbef61d6b7e1b',
      outputIndex: 0,
      script: ThoughtcoreLibDoge.Script(p2shAddress).toString(),
      notions: 3e9
    };
    ...
    it('1 input p2sh, 2 P2PKH outputs: ', () => {
      ...
      tx.from(p2shUtxoWithDOGE, [p2shPublicKey1, p2shPublicKey2], 2)
        .to([{ address: toAddress, notions: 1e9 }, { address: toAddress, notions: 2e8 }])
