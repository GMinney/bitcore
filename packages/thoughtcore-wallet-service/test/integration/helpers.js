...
helpers.signMessage = function(message, privKey) {
  var priv = new Thoughtcore.PrivateKey(privKey);
  const flattenedMessage = _.isArray(message)? _.join(message) : message;
  var hash = Utils.hashMessage(flattenedMessage);
  return Thoughtcore.crypto.ECDSA.sign(hash, priv, 'little').toString();
};

helpers.signRequestPubKey = function(requestPubKey, xPrivKey) {
  var priv = new Thoughtcore.HDPrivateKey(xPrivKey).deriveChild(Constants.PATHS.REQUEST_KEY_AUTH).privateKey;
...
helpers._generateCopayersTestData = function() {
  ...
  _.each(xPrivKeys, function(xPrivKeyStr, c) {
    var xpriv = Thoughtcore.HDPrivateKey(xPrivKeyStr);
    var xpub = Thoughtcore.HDPublicKey(xpriv);

    var xpriv_45H = xpriv.deriveChild(45, true);
    var xpub_45H = Thoughtcore.HDPublicKey(xpriv_45H);
    var id45 = Model.Copayer._xPubToCopayerId('tht', xpub_45H.toString());

    var xpriv_44H_0H_0H = xpriv.deriveChild(44, true).deriveChild(0, true).deriveChild(0, true);
    var xpub_44H_0H_0H = Thoughtcore.HDPublicKey(xpriv_44H_0H_0H);
    var id44tht = Model.Copayer._xPubToCopayerId('tht', xpub_44H_0H_0H.toString());
    var id44bch = Model.Copayer._xPubToCopayerId('bch', xpub_44H_0H_0H.toString());

    var xpriv_1H = xpriv.deriveChild(1, true);
    var xpub_1H = Thoughtcore.HDPublicKey(xpriv_1H);
    var priv = xpriv_1H.deriveChild(0).privateKey;
    var pub = xpub_1H.deriveChild(0).publicKey;

    console.log('{id44tht: ', "'" + id44tht + "',");
  ...
  server.createWallet(walletOpts, function(err, walletId) {
    ...
    async.eachSeries(_.range(n), function(i, cb) {
      ...
      if ((aliases && aliases.testnet && aliases.testnet == opts.network) || opts.network == 'testnet') {
        if (opts.coin == 'tht' || opts.coin == 'bch') {
...
helpers.randomTXID = function() {
  return Thoughtcore.crypto.Hash.sha256(Buffer.from((Math.random() * 100000).toString())).toString('hex');;
};

helpers.toSatoshi = function(tht) {
  if (_.isArray(tht)) {
    return _.map(tht, helpers.toSatoshi);
  } else {
    return Utils.strip(tht * 1e8);
  }
};
...
helpers._parseAmount = function(str) {
  ...
  var re = /^((?:\d+c)|u)?\s*([\d\.]+)\s*(tht|bit|sat)?$/;
...
helpers.stubUtxos = function(server, wallet, amounts, opts, cb) {
  ...
  var S = Thoughtcore_[wallet.coin].Script;
  async.waterfall([
...
helpers.stubFeeLevels = function(levels, fill, coin) {
  coin = coin || 'tht';
  let div = 1;
  if (coin == 'tht' || coin == 'bch' || coin == 'doge' || coin == 'ltc') {
    div = 1e8;  // thoughtd returns values in THT amounts
  }
...
helpers.clientSign = function(txp, derivedXPrivKey) {
  ...
  var xpriv = new Thoughtcore.HDPrivateKey(derivedXPrivKey, txp.network);
  ...
  switch(txp.coin) {
    ...
    case 'xrp':
      ...
      let tx = ChainService.getThoughtcoreTx(txp).uncheckedSerialize();
