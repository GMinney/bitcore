...
describe('Wallet service', function() {
      ...
      async.series([function(next) {
        ...
        server.getAdvert({ adId: '123' }, function(err, ad) {
          ...
          ad.signature.should.equal('304050302480413401348a3b34902403434512535e435463'),
            ad.app.should.equal('thoughtnetwork');
  ...
  describe('#createWallet', function() {
    ...
    it('should create wallet THT if n > 1 and BWC version is lower than 8.3.0', function(done) {
      var opts = {
        coin: 'tht',
        name: 'my wallet',
        m: 2,
    ...
    it('should check m-n combination', function(done) {
      ...
      async.eachSeries(pairs, function(pair, cb) {
        ...
        var pub = (new Thoughtcore.PrivateKey()).toPublicKey();
        opts.m = pair.m;
        opts.n = pair.n;
  ...
  describe('#joinWallet', function() {
    describe('New clients', function() {
      ...
      it('should join wallet THT if BWC version is lower than 8.3.0', function(done) {
        var copayerOpts = helpers.getSignedCopayerOpts({
          walletId: walletId,
          coin: 'tht',
          name: 'my wallet',
      ...
      helpers.stubUtxos(server, wallet, [1, 2], function() {
        ...
        helpers.createAndPublishTx(server, txOpts, TestData.copayers[0].privKey_1H_0, function(tx) {
          ...
          server.getStatus({}, function(err, status) {
            ...
            balance.lockedAmount.should.equal(tx.inputs[0].notions);
    ...
    describe('1-of-1 (BIP44 & P2PKH)', function() {
      beforeEach(function(done) {
        helpers.createAndJoinWallet(1, 1, function(s, w) {
          server = s;
          wallet = w;
          w.copayers[0].id.should.equal(TestData.copayers[0].id44tht);
          done();
        });
      });
    ...
    it('should get UTXOs for wallet addresses', function(done) {
      helpers.stubUtxos(server, wallet, [1, 2], function() {
        server.getUtxos({}, function(err, utxos) {
          ...
          _.sumBy(utxos, 'notions').should.equal(3 * 1e8);
          server.getMainAddresses({}, function(err, addresses) {
            var utxo = utxos[0];
    ...
    it('should skip dust UTXOs ', function(done) {
      helpers.stubUtxos(server, wallet, ['1 sat', 2, '10 sat', '100 sat', '1000 sat'], function() {
        server.getUtxos({}, function(err, utxos) {
          ...
          should.exist(utxos);
          utxos.length.should.equal(2);
          _.sumBy(utxos, 'notions').should.equal(2 * 1e8 + 1000);
          server.getMainAddresses({}, function(err, addresses) {
            var utxo = utxos[0];
            var address = _.find(addresses, {
              address: utxo.address
    ...
    it('should report no UTXOs if only dust', function(done) {
      helpers.stubUtxos(server, wallet, ['1 sat', '10 sat', '100 sat', '500 sat'], function() {
        server.getUtxos({}, function(err, utxos) {
          ...
          _.sumBy(utxos, 'notions').should.equal(0);
          done();
        });
      });
    });
  ...
  describe('Multiple request Pub Keys', function() {
    ...
    beforeEach(function() {
      reqPrivKey = new Thoughtcore.PrivateKey();
      ...
      var copayerId = Model.Copayer._xPubToCopayerId('tht', TestData.copayers[0].xPubKey_44H_0H_0H);
      opts = {
        copayerId: copayerId,
    ...
    describe('#addAccess 1-1', function() {
      ...
      it('should fail to access with wrong privkey after gaining access', function(done) {
        ws.addAccess(opts, function(err, res) {
          ...
          server.getBalance(res.wallet.walletId, function(err, bal) {
            should.not.exist(err);
            var privKey = new Thoughtcore.PrivateKey();
  ...
  describe('#getFeeLevels', function() {
    ...
    it('should get default fees if network cannot be accessed', function(done) {
      ...
      server.getFeeLevels({}, function(err, fees) {
        ...
        var defaults = _.fromPairs(_.map(Defaults.FEE_LEVELS['tht'], function(item) {
    ...
    it('should not store cache old fee levels after 1+ fail', function(done) {
      ...
      server.getFeeLevels({}, function(err, fees, cached) {
        ...
        fees.priority.feePerKb.should.equal(
          Defaults.FEE_LEVELS.tht[1].defaultValue
        );
    ...
    it('should get monotonically decreasing fee values', function(done) {
      _.find(Defaults.FEE_LEVELS['tht'], {
        nbBlocks: 6
    ...
    describe('#createTx ' + coin + ' flags' + JSON.stringify(flags), function() {
      ...
      describe('Tx proposal creation & publishing ' + coin, function() {
        ...
        describe('Validations', function() {
          ...
          if (['tht', 'bch', 'doge', 'ltc'].includes(coin)) {
            const coinAmount = {
              tht: 8000,
              bch: 8000,
              doge: 1e8,
          ...
          if (!flags.noUtxoTests) {
              ...
              it('should be able to specify change address', function(done) {
                helpers.stubUtxos(server, wallet, [1, 2], function(utxos) {
                  ...
                  server.createTx(txOpts, function(err, tx) {
                    should.not.exist(err);
                    should.exist(tx);
                    var t = ChainService.getThoughtcoreTx(tx);
            ...
            it('should be fail if specified change address is not from the wallet', function(done) {
              ...
              helpers.stubUtxos(server, wallet, [1, 2], function(utxos) {
                ...
                var addr = (new Thoughtcore_[coin].PrivateKey()).toAddress();
                var txOpts = {
                  outputs: [{
                    toAddress: addressStr,
                    amount: 1e8,
            ...
            it('should be able to specify inputs & absolute fee', function(done) {
              helpers.stubUtxos(server, wallet, [1, 2], function(utxos) {
                ...
                server.createTx(txOpts, function(err, tx) {
                  ...
                  t.getChangeOutput().notions.should.equal(3e8 - 1e8 - 1000e2);
                  done();
                });
              });
            });
        ...
        it('should support creating a tx with no change address', function(done) {
          ...
          helpers.stubUtxos(server, wallet, [1, 2], { coin }, function() {
            ...
            server.createTx(txOpts, function(err, txp) {
              should.not.exist(err);
              should.exist(txp);
              var t = ChainService.getThoughtcoreTx(txp).toObject();
              t.outputs.length.should.equal(1);
              t.outputs[0].notions.should.equal(max);
              done();
            });
          });
        });
        ...
        it('should fail gracefully when thoughtcore throws exception on raw tx creation', function(done) {
          const coinAmount = {
            tht: 0.5,
            bch: 0.5,
            eth: 0.5,
          ...
          helpers.stubUtxos(server, wallet, 2, { coin }, function() {
            ...
            sandbox.stub(Thoughtcore_.tht, 'Transaction').throws(new Error('dummy exception'));
            sandbox.stub(Thoughtcore_.bch, 'Transaction').throws(new Error('dummy exception'));
            sandbox.stub(Thoughtcore_.doge, 'Transaction').throws(new Error('dummy exception'));
            sandbox.stub(Thoughtcore_.ltc, 'Transaction').throws(new Error('dummy exception'));
            var txOpts = {
              outputs: [{
                toAddress: addressStr,
                amount: coinAmount[coin] * ts,
        ...
        if (!flags.noUtxoTests) {
          ...
          it('should fail to create a tx exceeding max size in kb', function(done) {
            Defaults.MAX_TX_SIZE_IN_KB_THT = 1;
            ...
            it('should create tx with 0 change output', function(done) {
              helpers.stubUtxos(server, wallet, 2, function() {
                ...
                server.createTx(txOpts, function(err, tx) {
                  should.not.exist(err);
                  should.exist(tx);
                  var thoughtcoreTx = ChainService.getThoughtcoreTx(tx);
                  thoughtcoreTx.outputs.length.should.equal(1);
                  thoughtcoreTx.outputs[0].notions.should.equal(tx.amount);
                  done();
                });
              });
            });
                ...
                server.createTx(txOpts, function(err, tx) {
                  ...
                  server.getPendingTxs({}, function(err, txs) {
                    ...
                    server.getBalance({}, function(err, balance) {
                      ...
                      balance.totalAmount.should.equal(3.6 * TO_SAT[coin]);
                      if (coin != 'eth') {
                        var amountInputs = _.sumBy(txs[0].inputs, 'notions');
        ...
        it('should be able to send max funds', function(done) {
          helpers.stubUtxos(server, wallet, [1, 2], { coin }, function() {
            ...
            server.createTx(txOpts, function(err, tx) {
              ...
              t.toObject().outputs[0].notions.should.equal(tx.amount);
              done();
            });
          });
        });
        ...
        if (!flags.noUtxoTests) {
              ...
              helpers.createAndPublishTx(server, txOpts, TestData.copayers[0].privKey_1H_0, function(txp) {
                ...
                server.signTx({
                  ...
                  server.broadcastTx({
                    ...
                    var t = ChainService.getThoughtcoreTx(txp);
          ...
          if (coin !== 'doge' && coin !== 'ltc') { // TODO
            it('should accept a tx proposal signed with a custom key', function(done) {
              var reqPrivKey = new Thoughtcore.PrivateKey();
          ...
          it('should shuffle outputs unless specified', function(done) {
            ...
            helpers.stubUtxos(server, wallet, amount, function() {
              ...
              server.createTx(txOpts, function(err, txp) {
                ...
                should.exist(txp);
                var t = ChainService.getThoughtcoreTx(txp);
                var changeOutput = t.getChangeOutput().notions;
                var outputs = _.without(_.map(t.outputs, 'notions'), changeOutput);
                ...
                server.createTx(txOpts, function(err, txp) {
                  ...
                  t = ChainService.getThoughtcoreTx(txp);
                  changeOutput = t.getChangeOutput().notions;
                  outputs = _.without(_.map(t.outputs, 'notions'), changeOutput);
    ...
    if (Constants.UTXO_CHAINS[coin.toUpperCase()]) {
      describe('UTXO Selection ' + coin, function() {
          ...
          helpers.stubUtxos(server, wallet, [1.3, 'u2', 'u0.1', 1.2], function(utxos) {
            ...
            server.createTx(txOpts, function(err, tx) {
              ...
              server.createTx(txOpts, function(err, tx) {
                ...
                err.message.should.include('Insufficient funds for fee. RequiredFee: 4500 Coin: tht feePerKb: 10000');
        ...
        it('should select a single utxo if within thresholds relative to tx amount', function(done) {
          helpers.stubUtxos(server, wallet, [1, '350bit', '100bit', '100bit', '100bit'], function() {
            ...
            server.createTx(txOpts, function(err, txp) {
              ...
              txp.inputs[0].notions.should.equal(35000);
        ...
        it('should return inputs in random order', function(done) {
          ...
          helpers.stubUtxos(server, wallet, _.range(1, 31), function(utxos) {
            var txOpts = {
              outputs: [{
                toAddress: '18PzpUFkFZE8zKWUPvfykkTxmB9oMR8qP7',
                amount: _.sumBy(utxos, 'notions') - 0.5e8,
              }],
              feePerKb: 100e2,
            };
            txOpts = Object.assign(txOpts, flags);
            server.createTx(txOpts, function(err, txp) {
              should.not.exist(err);
              should.exist(txp);
              var amounts = _.map(txp.inputs, 'notions');
        ...
        it('should select a confirmed utxos if within thresholds relative to tx amount', function(done) {
          helpers.stubUtxos(server, wallet, [1, 'u 350bit', '100bit', '100bit', '100bit'], function() {
            ...
            server.createTx(txOpts, function(err, txp) {
              ...
              txp.inputs[0].notions.should.equal(10000);
        ...
        it('should select smallest big utxo if small utxos are insufficient', function(done) {
          helpers.stubUtxos(server, wallet, [3, 1, 2, '100bit', '100bit', '100bit'], function() {
            ...
            server.createTx(txOpts, function(err, txp) {
              ...
              should.exist(txp);
              txp.inputs.length.should.equal(1);
              txp.inputs[0].notions.should.equal(1e8);
              done();
            });
          });
        });
        ...
        it('should select smallest big utxo if small utxos are below accepted ratio of txp amount', function(done) {
          helpers.stubUtxos(server, wallet, [9, 1, 1, 0.5, 0.2, 0.2, 0.2], function() {
            ...
            server.createTx(txOpts, function(err, txp) {
              ...
              should.exist(txp);
              txp.inputs.length.should.equal(1);
              txp.inputs[0].notions.should.equal(9e8);
              done();
            });
          });
        });
        it('should not fail with tx exceeded max size if there is at least 1 big input', function(done) {
          ...
          Defaults.MAX_TX_SIZE_IN_KB_THT = 2;
          ...
          helpers.stubUtxos(server, wallet, [100].concat(_.range(1, 20, 0)), function() {
            ...
            server.createTx(txOpts, function(err, txp) {
              ...
              should.exist(txp);
              txp.inputs.length.should.equal(1);
              txp.inputs[0].notions.should.equal(100e8);
              done();
            });
          });
        });
        ...
        it('should select unconfirmed utxos if not enough confirmed utxos', function(done) {
          helpers.stubUtxos(server, wallet, ['u 1tht', '0.5tht'], function() {
            var txOpts = {
              outputs: [{
                toAddress: '18PzpUFkFZE8zKWUPvfykkTxmB9oMR8qP7',
                amount: 0.8e8,
        ...
        it('should correct fee if resulting change would be below threshold', function(done) {
          helpers.stubUtxos(server, wallet, ['180bit', '500sat'], function() {
            ...
            server.createTx(txOpts, function(err, txp) {
              should.not.exist(err);
              txp.inputs.length.should.equal(1);
              (_.sumBy(txp.inputs, 'notions') - txp.outputs[0].amount - txp.fee).should.equal(0);
              var changeOutput = ChainService.getThoughtcoreTx(txp).getChangeOutput();
    ...
    it('should create a BCH tx proposal with cashaddr outputs (w/ prefix) and return Copay addr', function(done) {
      ...
      helpers.createAndJoinWallet(1, 1, {
        ...
        helpers.stubUtxos(s, w, [1, 2], function() {
          var txOpts = {
            outputs: [{
              toAddress: 'thoughtcash:' + cashAddr,
              amount: amount,
            }],
            message: 'some message',
            customData: 'some custom data',
        ...
        helpers.createAddresses(s, w, 1, 1, function(mainAddresses, changeAddress) {
          helpers.stubUtxos(s, w, [1, 2], function() {
            var txOpts = {
              ...
              customData: 'some custom data',
              feePerKb: 123e2,
              changeAddress: 'thoughtcash:' + changeAddress[0].address,
            };
            s.createTx(txOpts, function(err, tx) {
    ...
    it('should return inputs in random order', function(done) {
      ...
      helpers.stubUtxos(server, wallet, _.range(1, 31), function(utxos) {
        server.getSendMaxInfo({
          ...
          should.not.exist(err);
          should.exist(info);
          var amounts = _.map(info.inputs, 'notions');
          amounts.length.should.equal(30);
          _.every(amounts, function(amount, i) {
            if (i == 0) return true;
            return amount < amounts[i - 1];
  ...
  describe('Check requiredFeeRate  THT', function() {
    var server, wallet;
    ...
    const cases = [
      ...
      },
      {
        ...
        vSize: 141, // from https://tht.com/tools/tx/decode
      },
      {
      ...
      {
        n: 2,
        vSize: 123, // from https://tht.com/tools/tx/decode
        name: 'Segwit, sendmax',
        requiredFeeRate: 10000,
        sendMax: true,
        fromSegwit: true,
    ...
    function checkTx(txOpts, x, cb) {
      ...
      helpers.createAndPublishTx(server, txOpts, TestData.copayers[0].privKey_1H_0, function(tx) {
        sign(0, tx, (txp) => {
          ...
          t.getFee().should.be.gte(CWC.ThoughtcoreLibDoge.Transaction.DUST_AMOUNT);

          if (t.getFee() > CWC.ThoughtcoreLibDoge.Transaction.DUST_AMOUNT) {
            // size should be above (or equal) the required FeeRate
            actualFeeRate.should.not.be.below(x.requiredFeeRate);
            actualFeeRate.should.be.below(x.requiredFeeRate * 1.5); // no more that 50% extra
          }
    ...
    it('should broadcast a tx and set locktime & version', function(done) {
      ...
      server.broadcastTx({
        ...
        server.getTx({
          ...
          should.exist(txp.raw);
          const tx = new Thoughtcore.Transaction(txp.raw);
          clock.restore();
          done();
        });
      });
  ...
  describe('#getWalletFromIdentifier', function() {
    ...
    it('should get wallet from tx proposal', function(done) {
      ...
      helpers.stubUtxos(server, wallet, '1 tht', function() {
        var txOpts = {
          outputs: [{
            toAddress: '18PzpUFkFZE8zKWUPvfykkTxmB9oMR8qP7',
            amount: 1000e2
  ...
  describe('THT & BCH wallets with same seed', function() {
    var server = {},
      wallet = {};
    beforeEach(function(done) {
      helpers.createAndJoinWallet(1, 1, function(s, w) {
    ...
    it('should create address', function(done) {
      server.tht.createAddress({}, function(err, address) {
        should.not.exist(err);
        should.exist(address);
        address.walletId.should.equal(wallet.tht.id);
        address.coin.should.equal('tht');
        address.network.should.equal('livenet');
        address.address.should.equal('1L3z9LPd861FWQhf3vDn89Fnc9dkdBo2CG');
        server.bch.createAddress({}, function(err, address) {
          ...
          address.network.should.equal('livenet');
          address.address.should.equal('qrg04mz8h67j9dck3f3f3sa560taep87yqnwra9ak6');
          server.tht.getMainAddresses({}, function(err, addresses) {
            should.not.exist(err);
            addresses.length.should.equal(1);
            addresses[0].coin.should.equal('tht');
            addresses[0].walletId.should.equal(wallet.tht.id);
      ...
      describe('User logged in', () => {
        it('should return swap crypto disabled if the user is registred in NY and located in NY', () => {
          const opts = {
            currentLocationCountry: 'US',
            currentLocationState: 'NY',
            thoughtnetworkIdLocationCountry: 'US',
            thoughtnetworkIdLocationState: 'NY',
          };
        ...
        it('should return swap crypto enabled if the user is registred outside NY and located in NY', () => {
          const opts = {
            ...
            currentLocationState: 'NY',
            thoughtnetworkIdLocationCountry: 'US',
            thoughtnetworkIdLocationState: 'FL',
          };
        ...
        it('should return swap crypto enabled if the user is registred in other country than USA and located in NY', () => {
          const opts = {
            currentLocationCountry: 'US',
            currentLocationState: 'NY',
            thoughtnetworkIdLocationCountry: 'AR',
            thoughtnetworkIdLocationState: 'T',
          };
        ...
        it('should return swap crypto disabled if platform is ios and version of the app is 14.11.5', () => {
          const opts = {
            ...
            thoughtnetworkIdLocationCountry: 'US',
            thoughtnetworkIdLocationState: 'GA',
            platform: {
              os: 'ios',
              version: '1.1.1'
            },
    ...
    describe('#moonpayGetSignedPaymentUrl', () => {
      beforeEach(() => {
        req = {
          ...
          body: {
            env: 'production',
            currencyCode: 'tht',
            walletAddress: 'thought:123123',
            baseCurrencyCode: 'usd',
            baseCurrencyAmount: '500',
            externalTransactionId: '123123',
            redirectURL: 'thoughtnetwork://moonpay'
          }
        }
      });
      ...
      it('should get the paymentUrl properly if req is OK', () => {
        try {
          ...
          data.urlWithSignature.should.equal('widgetApi2?apiKey=apiKey2&currencyCode=tht&walletAddress=thought%3A123123&baseCurrencyCode=usd&baseCurrencyAmount=500&externalTransactionId=123123&redirectURL=thoughtnetwork%3A%2F%2Fmoonpay&signature=%2FDnbsboySgE%2FeAvMrwzROCLuuctkhgw5C2t2OofjOzo%3D');
        } catch (err) {
          should.not.exist(err);
        }
      });
    ...
    describe('#moonpayGetSellSignedPaymentUrl', () => {
      beforeEach(() => {
        req = {
          ...
          body: {
            env: 'production',
            baseCurrencyCode: 'tht',
            baseCurrencyAmount: 500,
            externalTransactionId: '123123',
            redirectURL: 'thoughtnetwork://moonpay',
            quoteCurrencyCode: 'usd',
            refundWalletAddress: 'thought:123123',
          }
        }
      });
      ...
      it('should get the paymentUrl properly if req is OK', () => {
        try {
          ...
          should.exist(data.urlWithSignature);
          data.urlWithSignature.should.equal('sellWidgetApi2?apiKey=apiKey2&baseCurrencyCode=tht&baseCurrencyAmount=500&externalTransactionId=123123&redirectURL=thoughtnetwork%3A%2F%2Fmoonpay&quoteCurrencyCode=usd&refundWalletAddress=thought%3A123123&signature=otiVaKVxKT%2BRNOfkSMOk07U3JxY4DrpPAztiXl5Wvjc%3D');
        } catch (err) {
          should.not.exist(err);
        }
      });
  ...
  describe('Sardine', () => {
    ...
    describe('#sardineGetSupportedTokens', () => {
      ...
      it('should work properly if req is OK', async() => {
        server.request = fakeRequest;
        try {
          const data = await server.sardineGetSupportedTokens(req);
          should.exist(data);
        } catch (err) {
          should.not.exist(err);
        }
    ...
    describe('#rampGetSignedPaymentUrl', () => {
      beforeEach(() => {
        req = {
          ...
          body: {
            ...
            enabledFlows: 'ONRAMP',
            defaultFlow: 'ONRAMP',
            userAddress: 'thought:123123',
            selectedCountryCode: 'US',
            defaultAsset: 'THT_THT',
            finalUrl: 'thoughtnetwork://ramp',
          }
        }
      });
      ...
      it('should get the paymentUrl properly if req is OK', () => {
        try {
          ...
          should.exist(data.urlWithSignature);
          data.urlWithSignature.should.equal('widgetApi2?hostApiKey=apiKey2&swapAsset=THT_THT&userAddress=thought%3A123123&selectedCountryCode=US&finalUrl=thoughtnetwork%3A%2F%2Framp&enabledFlows=ONRAMP&defaultFlow=ONRAMP&swapAmount=1000000&defaultAsset=THT_THT');
        } catch (err) {
          should.not.exist(err);
        }
      });
      ...
      it('should get the paymentUrl properly if req is OK for web', () => {
        try {
          ...
          const data = server.rampGetSignedPaymentUrl(req);
          should.exist(data.urlWithSignature);
          data.urlWithSignature.should.equal('widgetApi4?hostApiKey=apiKey4&swapAsset=THT_THT&userAddress=thought%3A123123&selectedCountryCode=US&finalUrl=thoughtnetwork%3A%2F%2Framp&defaultAsset=THT_THT');
        } catch (err) {
          should.not.exist(err);
        }
      });
  ...
  describe('Transak', () => {
    ...
    describe('#transakGetAccessToken', () => {
      ...
      it('should work properly if req is OK', async () => {
        server.request = fakeRequest;
        try {
          const data = await server.transakGetAccessToken(req);
          should.exist(data);
        } catch (err) {
          should.not.exist(err);
        }
    ...
    describe('#transakGetCryptoCurrencies', () => {
      ...
      it('should work properly if req is OK', async () => {
        server.request = fakeRequest;
        try {
          const data = await server.transakGetCryptoCurrencies(req);
          should.exist(data);
        } catch (err) {
          should.not.exist(err);
        }
    ...
    describe('#transakGetFiatCurrencies', () => {
      ...
      it('should work properly if req is OK', async () => {
        server.request = fakeRequest;
        try {
          const data = await server.transakGetFiatCurrencies(req);
          should.exist(data);
        } catch (err) {
          should.not.exist(err);
        }
    ...
    describe('#transakGetQuote', () => {
      ...
      it('should work properly if req is OK', async () => {
        server.request = fakeRequest;
        try {
          const data = await server.transakGetQuote(req);
          should.exist(data);
        } catch (err) {
          should.not.exist(err);
        }
    ...
    describe('#transakGetSignedPaymentUrl', () => {
      beforeEach(() => {
        req = {
          ...
          body: {
            ...
            redirectURL: 'thoughtnetwork://transak',
            fiatAmount: '500',
            fiatCurrency: 'USD',
            network: 'mainnet',
            cryptoCurrencyCode: 'THT',
      ...
      it('should get the paymentUrl properly if req is OK', () => {
        try {
          const data = server.transakGetSignedPaymentUrl(req);
          should.exist(data.urlWithSignature);
          data.urlWithSignature.should.equal('widgetApi2?apiKey=apiKey2&walletAddress=walletAddress1&redirectURL=thoughtnetwork%3A%2F%2Ftransak&fiatAmount=500&fiatCurrency=USD&network=mainnet&cryptoCurrencyCode=THT&partnerOrderId=partnerOrderId1&partnerCustomerId=partnerCustomerId1');
        } catch (err) {
          should.not.exist(err);
        }
      });
      ...
      it('should get the paymentUrl properly if req is OK for web', () => {
        try {
          ...
          const data = server.transakGetSignedPaymentUrl(req);
          should.exist(data.urlWithSignature);
          data.urlWithSignature.should.equal('widgetApi4?apiKey=apiKey4&walletAddress=walletAddress1&redirectURL=thoughtnetwork%3A%2F%2Ftransak&fiatAmount=500&fiatCurrency=USD&network=mainnet&cryptoCurrencyCode=THT&partnerOrderId=partnerOrderId1&partnerCustomerId=partnerCustomerId1');
        } catch (err) {
          should.not.exist(err);
        }
      });
    ...
    describe('#transakGetOrderDetails', () => {
      ...
      it('should work properly if req is OK', async () => {
        server.request = fakeRequest;
        try {
          const data = await server.transakGetOrderDetails(req);
          should.exist(data);
        } catch (err) {
          should.not.exist(err);
        }
