...
var BWS = require('thoughtcore-wallet-service');
...
describe('client API', function () {
  ...
  describe('Client Internals', () => {
    it('should expose thoughtcore', () => {
      should.exist(Thoughtcore);
      should.exist(Thoughtcore.HDPublicKey);
    });
  });
  // todo
  describe('Server internals', () => {
    ...
    it('should handle critical errors', done => {
      ...
      expressApp.start(
        ...
        () => {
          ...
          var client = helpers.newClient(app);
          client.storage = s2;
          client.fromString(k.createCredentials(null, { coin: 'tht', n: 1, network: 'testnet', account: 0 }));
          client.createWallet(
            '1',
            '2',
            1,
    ...
    it('should handle critical errors (Case5)', done => {
      ...
      clients[0].fromString(k.createCredentials(null, { coin: 'tht', n: 1, network: 'testnet', account: 0 }));
  ...
  describe('Build & sign txs', () => {
    ...
    var derivedPrivateKey = {
      BIP44: new Thoughtcore.HDPrivateKey(masterPrivateKey).deriveChild("m/44'/1'/0'").toString(),
      BIP45: new Thoughtcore.HDPrivateKey(masterPrivateKey).deriveChild("m/45'").toString(),
      BIP48: new Thoughtcore.HDPrivateKey(masterPrivateKey).deriveChild("m/48'/1'/0'").toString()
    };
    ...
    describe('#buildTx', () => {
      it('Raw tx roundtrip', () => {
        ...
        var publicKeyRing = [
          {
            xPubKey: new Thoughtcore.HDPublicKey(derivedPrivateKey['BIP44'])
          }
        ];
        ...
        var t2 = new Thoughtcore.Transaction(t);
        t2.inputs.length.should.equal(2);
        t2.outputs.length.should.equal(2);
        t2.outputs[0].notions.should.equal(1200);
      });
      it('should build a tx correctly (BIP44)', () => {
        ...
        var thoughtcoreError = t.getSerializationError({
          disableIsFullySigned: true,
          disableSmallFees: true,
          disableLargeFees: true
        });

        should.not.exist(thoughtcoreError);
      ...
      it('should build a P2WPKH tx correctly (BIP44)', () => {
        ...
        const toAddress = Utils.deriveAddress('P2WPKH', publicKeyRing, 'm/0/0', 1, 'livenet', 'tht');
        const changeAddress = Utils.deriveAddress('P2WPKH', publicKeyRing, 'm/0/1', 1, 'livenet', 'tht');
      ...
      it('should build a P2WSH tx correctly (BIP48)', () => {
        var publicKeyRing = [
          {
            xPubKey: new Thoughtcore.HDPublicKey(derivedPrivateKey['BIP48'])
          }
        ];

        const toAddress = Utils.deriveAddress('P2WSH', publicKeyRing, 'm/0/0', 1, 'livenet', 'tht');
        const changeAddress = Utils.deriveAddress('P2WSH', publicKeyRing, 'm/0/1', 1, 'livenet', 'tht');
      ...
      it('should build an eth txp correctly', () => {
        ...
        const publicKeyRing = [
          {
            xPubKey: new Thoughtcore.HDPrivateKey(masterPrivateKey).deriveChild(path).toString()
          }
        ];
      ...
      it('should build a tx with provided output scripts', () => {
        ...
        t.outputs[0].notions.should.equal(txp.outputs[0].amount);
        t.outputs[1].script.toHex().should.equal(txp.outputs[1].script);
        t.outputs[1].notions.should.equal(txp.outputs[1].amount);
        t.outputs[2].script.toHex().should.equal(txp.outputs[2].script);
        t.outputs[2].notions.should.equal(txp.outputs[2].amount);
        var changeScript = Thoughtcore.Script.fromAddress(txp.changeAddress.address).toHex();
    ...
    describe('#pushSignatures', () => {
      it('should sign BIP45 P2SH correctly', () => {
        ...
        var publicKeyRing = [
          {
            xPubKey: new Thoughtcore.HDPublicKey(derivedPrivateKey['BIP45'])
          }
        ];
        ...
        // This is a GOOD test, since thoughtcore ONLY accept VALID signatures
      ...
      it('should sign tht proposal correctly', () => {
        var toAddress = 'msj42CCGruhRsFrGATiUuh25dtxYtnpbTx';
        var changeAddress = 'msj42CCGruhRsFrGATiUuh25dtxYtnpbTx';
      ...
      it('should sign tht proposal correctly (tx V2)', () => {
        var toAddress = 'msj42CCGruhRsFrGATiUuh25dtxYtnpbTx';
        var changeAddress = 'msj42CCGruhRsFrGATiUuh25dtxYtnpbTx';
  ...
  describe('Wallet secret round trip', () => {
    it('should create secret and parse secret', () => {
      ...
      while (i++ < 100) {
        ...
        var walletPrivKey = new Thoughtcore.PrivateKey();
        var network = i % 2 == 0 ? 'testnet' : 'livenet';
        var coin = i % 3 == 0 ? 'bch' : 'tht';
  ...
  describe('Wallet Creation', () => {
    ...
    it('should be able to access wallet name in non-encrypted wallet (legacy)', done => {
      ...
      var wpk = new Thoughtcore.PrivateKey();
      var args = {
        name: 'mywallet',
        m: 1,
        n: 1,
      ...
      helpers.createAndJoinWallet(clients, keys, 2, 3, {}, () => {
        helpers.tamperResponse(
          ...
          status => {
            ...
            status.wallet.copayers[1].xPubKey = new Thoughtcore.HDPrivateKey().publicKey;
      ...
      async.map(
        ...
        (err, addresses) => {
          ...
          clients[0].getUtxos(opts, (err, utxos) => {
            should.not.exist(err);
            utxos.length.should.equal(1);
            _.sumBy(utxos, 'notions').should.equal(1 * 1e8);
            done();
          });
        }
      );
  ...
  describe('Network fees', () => {
    it('should get current fee levels for THT', done => {
      ...
      clients[0].credentials = {};
      clients[0].getFeeLevels('tht', 'livenet', (err, levels) => {
        should.not.exist(err);
        should.exist(levels);
    ...
    it('should return data with inputs', done => {
      ...
      clients[0].getSendMaxInfo(opts, (err, result) => {
        ...
        _.each(result.inputs, i => {
          totalNotions = totalNotions + i.notions;
        });
        result.amount.should.be.equal(totalNotions - result.fee);
        done();
      });
    });
  });
      ...
      helpers.createAndJoinWallet(
        ...
        w => {
          clients[0].createAddress((err, address) => {
            ...
            if (coin == 'bch') {
              address.address = Thoughtcore_['bch'].Address(address.address).toString(true);
            }
            // ==
  ...
  describe('Payment Protocol V2', function () {
    ...
    let tests = [
/*      {
        name: 'weightedSize: Legacy THT',
        opts: { network: 'livenet' },
        expectedSize: 220,
      },
      {
        name: 'weightedSize: Segwit THT',
 ...
 */   {
      ...
      describe(x.name, () => {
        ...
        beforeEach(async () => {
          await new Promise(resolve => {
            PP = TestData.payProJsonV2.tht;
            DATA = JSON.parse(TestData.payProJsonV2Body.tht);

            mockRequest(Buffer.from(TestData.payProJsonV2.tht.body, 'hex'), TestData.payProJsonV2.tht.headers);
            helpers.createAndJoinWallet(clients, keys, 2, 2, x.opts, w => {
              clients[0].createAddress((err, x0) => {
                ...
                blockchainExplorerMock.setUtxo(x0, 1, 2);
                var opts = {
                  paymentUrl: 'https://thoughtnetwork.com/i/LanynqCPoL2JQb8z8s5Z3X'
                };
    ...
    describe('Shared wallet THT', () => {
      ...
      it('Should Create and Verify a Tx from PayPro', done => {
        clients[1].getTxProposals({}, (err, txps) => {
          try {
            ...
            tx.payProUrl.should.equal('https://thoughtnetwork.com/i/LanynqCPoL2JQb8z8s5Z3X');
            done();
          } catch (err) {
            console.error(err);
          }
      ...
      it('Should handle broken paypro data', async () => {
        mockRequest(Buffer.from('broken data'), TestData.payProJsonV2.tht.headers);
        var opts = {
          payProUrl: 'dummy'
        };
          ...
          clients[0].pushSignatures(txps[0], signatures, (err, xx, paypro) => {
            ...
            clients[1].pushSignatures(xx, signatures2, (err, yy, paypro) => {
              ...
              clients[1].broadcastTxProposal(yy, (err, zz, memo) => {
                ...
                spy.called.should.be.true;
                memo.should.equal(
                  'Payment request for Thought invoice LanynqCPoL2JQb8z8s5Z3X for merchant Thought Visa® Load (USD-USA)'
                );
          ...
          clients[0].pushSignatures(txps[0], signatures, (err, xx, paypro) => {
            ...
            clients[1].pushSignatures(xx, signatures, (err, yy, paypro) => {
              ...
              clients[1].broadcastTxProposal(yy, (err, zz, memo) => {
                ...
                var tx = new Thoughtcore.Transaction(rawTx);
    ...
    describe('Shared wallet / requiredFeeRate THT', () => {
      beforeEach(async () => {
        await new Promise(resolve => {
          DATA = JSON.parse(TestData.payProJsonV2Body.tht);
          mockRequest(Buffer.from(TestData.payProJsonV2.tht.body, 'hex'), TestData.payProJsonV2.tht.headers);
            ...
            clients[1].pushSignatures(xx, signatures, (err, yy, paypro) => {
              ...
              clients[1].broadcastTxProposal(yy, (err, zz, memo) => {
                try {
                  should.not.exist(err);
                  spy.called.should.be.true;
                  postArgs[1].currency.should.equal('THT');
        ...
        await new Promise(resolve => {
          ...
          helpers.createAndJoinWallet(clients, keys, 1, 1, { coin: 'bch', network: 'livenet' }, w => {
            clients[0].createAddress(async (err, x0) => {
              ...
              // TODO change createAddress to /v4/, and remove this.
              //x0.address = Thoughtcore_['bch'].Address(x0.address).toString(true);
              // ======
              blockchainExplorerMock.setUtxo(x0, 1, 2);
              blockchainExplorerMock.setUtxo(x0, 1, 2);
              var opts = {
                paymentUrl: 'https://thoughtnetwork.com/i/XM8XbreRs6cnKkR3yYT6qQ',
                chain: 'BCH',
                currency: 'BCH'
              };
              try {
        ...
        clients[0].getTxProposals({}, (err, txps) => {
          ...
          clients[0].pushSignatures(txps[0], signatures, (err, xx, paypro) => {
            ...
            clients[0].broadcastTxProposal(xx, (err, zz, memo) => {
              ...
              spy.called.should.be.true;
              var rawTx = Buffer.from(postArgs[1].transactions[0].tx, 'hex');
              var tx = Thoughtcore_['bch'].Transaction(rawTx);
              var script = tx.inputs[0].script;
              script.isPublicKeyHashIn().should.equal(true);
              memo.should.be.equal(
                'Payment request for Thought invoice XM8XbreRs6cnKkR3yYT6qQ for merchant Thought Visa® Load (USD-USA)'
              );
              done();
            });
          });
          ...
          clients[0].pushSignatures(x2, signatures, (err, txp) => {
            ...
            if (doBroadcast) {
              clients[0].broadcastTxProposal(txp, (err, txp) => {
                ...
                txp.txid.should.equal(new Thoughtcore.Transaction(blockchainExplorerMock.lastBroadcasted).id);
                done();
              });
            } else {
              done();
  ...
  describe('Mobility, backup & restore', () => {
    ...
    describe('Recovery', () => {
              ...
              Client.serverAssistedImport(
                ...
                (err, k, c) => {
                  ...
                  recoveryClient.openWallet(err => {
                    ...
                    recoveryClient2.openWallet(err => {
                      should.not.exist(err);
                      recoveryClient2.credentials.coin.should.equal('wtht');
                      should.exist(recoveryClient2.credentials.chain);
            ...
            clients[0].createAddress((err, addr) => {
              ...
              Client.serverAssistedImport(
                ...
                (err, k, c) => {
                  should.not.exist(err);
                  c.length.should.equal(2);
                  c[0].credentials.coin.should.equal('tht');
      ...
      it('should be able to gain access to three tht 1-1 accounts of a single wallet from mnemonic and add wallet info correctly to all of them', done => {
        let key = new Key({ seedType: 'new' });
        helpers.createAndJoinWallet(clients, keys, 1, 1, { key: key }, () => {
              ...
              clients[0].createAddress((err, addr) => {
                ...
                Client.serverAssistedImport(
                  ...
                  (err, k, c) => {
                    ...
                    c.length.should.equal(3);
                    c[0].credentials.coin.should.equal('tht');
                    c[1].credentials.coin.should.equal('tht');
                    c[2].credentials.coin.should.equal('tht');
      ...
      it('should be able to gain access to seven tht 1-1 accounts of a single wallet from mnemonic', done => {
        let key = new Key({ seedType: 'new' });
        helpers.createAndJoinWallet(clients, keys, 1, 1, { key: key }, () => {
          helpers.createAndJoinWallet(clients, keys, 1, 1, { key: key, account: 1 }, () => {
            helpers.createAndJoinWallet(clients, keys, 1, 1, { key: key, account: 2 }, () => {
                        ...
                        Client.serverAssistedImport(
                          ...
                          (err, k, c) => {
                            ...
                            for (let i = 0; i < c.length; i++) {
                              c[i].credentials.coin.should.equal('tht');
    ...
    describe('1-1 wallets', () => {
      ...
      beforeEach(done => {
        ...
        helpers.createAndJoinWallet(clients, keys, 1, 1, {}, () => {
          clients[0].createAddress((err, x0) => {
            ...
            var k = new Thoughtcore.PrivateKey();
      ...
      it('should add access with copayer name', done => {
        ...
        clients[0].addAccess(opts2, (err, x, key) => {
          ...
          var k = new Thoughtcore.PrivateKey(key);
          var c = clients[0].credentials;
          c.requestPrivKey = k.toString();
          c.requestPubKey = k.toPublicKey().toString();
  ...
  var addrMap = {
    tht: ['1PuKMvRFfwbLXyEPXZzkGi111gMUCs6uE3', '1GG3JQikGC7wxstyavUBDoCJ66bWLLENZC'],
        ...
        helpers.createAndJoinWallet(clients, keys, 1, 1, { coin: coin, network: 'livenet' }, () => {
          ...
          clients[0].buildTxFromPrivateKey(
            ...
            (err, tx) => {
              ...
              output.notions.should.equal(123 * 1e8 - 10000);
