...
const helpers = {
    toSatoshi: tht => {
        if (_.isArray(tht)) {
            ...
            return parseFloat((tht * 1e8).toPrecision(12));
        }
    },
    newClient: app => {
        $.checkArgument(app);
        ...
        var utxos = _.map(amounts, (amount, i) => {
            ...
            switch (scriptType) {
                ...
                case Constants.SCRIPT_TYPES.P2SH:
                    scriptPubKey = new Thoughtcore.Script.buildMultisigOut(address.publicKeys, requiredSignatures).toScriptHashOut();
                    break;
                case Constants.SCRIPT_TYPES.P2WPKH:
                case Constants.SCRIPT_TYPES.P2PKH:
                    scriptPubKey = new Thoughtcore.Script.buildPublicKeyHashOut(address.address);
                    break;
            }
            should.exist(scriptPubKey);

            var obj = {
                txid: new Thoughtcore.crypto.Hash.sha256(Buffer.alloc(i)).toString('hex'),
                vout: 100,
                notions: helpers.toSatoshi(amount),
...
const blockchainExplorerMock = {
    ...
    setUtxo: (address, amount, m, confirmations) => {
        var B = Thoughtcore_[address.coin];
        var scriptPubKey;
        switch (address.type) {
            case Constants.SCRIPT_TYPES.P2SH:
        ...
        blockchainExplorerMock.utxos.push({
            txid: new Thoughtcore.crypto.Hash.sha256(Buffer.alloc(Math.random() * 100000)).toString('hex'),
            outputIndex: 0,
            amount: amount,
            notions: amount * 1e8,
            address: address.address,
    ...
    broadcast: (raw, cb) => {
        ...
        let hash;
        try {
            let tx = new Thoughtcore.Transaction(raw);
