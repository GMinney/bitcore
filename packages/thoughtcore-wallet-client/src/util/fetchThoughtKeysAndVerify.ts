...
keyRequests.push(
  (() => {
    console.log('Fetching keys from github.com/thoughtnetwork/pgp-keys...');
    return request({
      method: 'GET',
      url: 'https://api.github.com/repos/thoughtnetwork/pgp-keys/contents/keys',
      headers: {
        'user-agent': 'Thought Key-Check Utility'
      },
      json: true
    }).then(pgpKeyFiles => {
        ...
        fileDataPromises.push(
          (() => {
            return request({
              ...
              let hash = crypt.createHash('sha256').update(body).digest('hex');
...
keyRequests.push(
  (() => {
    console.log('Fetching keys from thoughtnetwork.com/pgp-keys...');
    return request({
      method: 'GET',
      url: 'https://thoughtnetwork.com/pgp-keys.json',
      headers: {
        'user-agent': 'Thought Key-Check Utility'
      },
      json: true
      ...
      body.pgpKeys.forEach(function (key) {
        let hash = crypt
          ...
          .digest('hex');
        thoughtnetworkPgpKeys[hash] = key.publicKey;
      });
      return Promise.resolve();
    });
  })()
...
Promise.all(keyRequests)
  .then(() => {
    if (
      Object.keys(githubPgpKeys).length !== Object.keys(thoughtnetworkPgpKeys).length
    ) {
      console.log('Warning: Different number of keys returned by key lists');
    }

    let thoughtnetworkOnlyKeys = Object.keys(thoughtnetworkPgpKeys).filter(keyHash => {
      return !githubPgpKeys[keyHash];
    });

    let githubOnlyKeys = Object.keys(githubPgpKeys).filter(keyHash => {
      return !thoughtnetworkPgpKeys[keyHash];
    });

    if (thoughtnetworkOnlyKeys.length) {
      console.log('Thought returned some keys which are not present in github');
      Object.keys(thoughtnetworkOnlyKeys).forEach(keyHash => {
        console.log(`Hash ${keyHash} Key: ${thoughtnetworkOnlyKeys[keyHash]}`);
      });
    }

    if (githubOnlyKeys.length) {
      console.log('GitHub returned some keys which are not present in Thought');
  ...
  .then(() => {
    ...
    return Promise.all(
      Object.values(thoughtnetworkPgpKeys).map(pgpKeyString => {
  ...
  .then(() => {
    console.log(
      'Fetching current ECC keys from thoughtnetwork.com/signingKeys/paymentProtocol.json'
    );
    return request({
      method: 'GET',
      url: 'https://thoughtnetwork.com/signingKeys/paymentProtocol.json',
      headers: {
        'user-agent': 'Thought Key-Check Utility'
      }
    }).then(rawEccPayload => {
      if (rawEccPayload.indexOf('rate limit') !== -1) {
        return Promise.reject('Rate limited by Thought');
      }
  ...
  .then(() => {
    ...
    return request({
      method: 'GET',
      url: `https://thoughtnetwork.com/signatures/${eccKeysHash}.json`,
      headers: {
        'user-agent': 'Thought Key-Check Utility'
      },
      json: true
  ...
  .then(() => {
    if (signatureCount >= Object.keys(thoughtnetworkPgpKeys).length / 2) {
      ...
      console.log('----\nValid keymap for use in thoughtRpc example:');

      parsedEccPayload.publicKeys.forEach(pubkey => {
        // Here we are just generating the pubkey hash (tht address) of each of the public keys received for easy lookup later
        // as this is what will be provided by the x-identity header
        let a = crypt.createHash('sha256').update(pubkey, 'hex').digest();
        let b = crypt.createHash('rmd160').update(a).digest('hex');
        let c = '00' + b; // This is assuming livenet
        let d = crypt.createHash('sha256').update(c, 'hex').digest();
        let e = crypt.createHash('sha256').update(d).digest('hex');
        ...
        // Add Thoughtnetwork's testnet
        keyMap['mh65MN7drqmwpCRZcEeBEE9ceQCQ95HtZc'] = {
          owner: 'Thought (TESTNET ONLY - DO NOT TRUST FOR ACTUAL THOUGHT)',
          networks: ['test'],
          domains: ['test.thoughtnetwork.com'],
          publicKey:
