...
var Thoughtcore = ThoughtcoreLib;
...
export class Key {
  ...
  constructor(
    ...
    switch (opts.seedType) {
      ...
      case 'extendedPrivateKey':
        ...
        let xpriv;
        try {
          xpriv = new Thoughtcore.HDPrivateKey(x);
        } catch (e) {
  ...
  get = function (password) {
    ...
    if (this.isPrivKeyEncrypted()) {
      ...
      try {
        ...
        if (!this.fingerPrint) {
          let xpriv = new Thoughtcore.HDPrivateKey(keys.xPrivKey);
          this.fingerPrint = xpriv.fingerPrint.toString('hex');
  ...
  createCredentials = function (password, opts) {
    ...
    if (['testnet', 'regtest'].includes(opts.network)) {
      // Hacky: THT/BCH xPriv depends on network: This code is to
      ...
      xPrivKey = new Thoughtcore.HDPrivateKey(x);
    }
  ...
  createAccess = function (password, opts) {
    ...
    var requestPrivKey = new Thoughtcore.PrivateKey(opts.requestPrivKey || null);
  ...
  sign = function (rootPath, txp, password, cb) {
    ...
    var xpriv = new Thoughtcore.HDPrivateKey(derived);
