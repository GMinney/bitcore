...
const Thoughtcore = ThoughtcoreLib;
const Thoughtcore_ = {
  tht: Thoughtcore,
...
const PublicKey = Thoughtcore.PublicKey;
...
export class Utils {
  ...
  static getChain(coin: string): string {
    try {
      ...
      if (
        Constants.THOUGHTNETWORK_SUPPORTED_ETH_ERC20.includes(normalizedChain) ||
      ...
      return 'tht'; // coin should always exist but most unit test don't have it -> return tht as default
    }
  }
  ...
  static privateKeyToAESKey(privKey) {
    ...
    $.checkArgument(
      Thoughtcore.PrivateKey.isValid(privKey),
      'The private key received is invalid'
    );
    var pk = Thoughtcore.PrivateKey.fromString(privKey);
    return Thoughtcore.crypto.Hash.sha256(pk.toBuffer())
      .slice(0, 16)
      .toString('base64');
  }
  ...
  static deriveAddress(
    ...
    chain = chain || 'tht';
    var thoughtcore = Thoughtcore_[chain];
    var publicKeys = _.map(publicKeyRing, item => {
      var xpub = new thoughtcore.HDPublicKey(item.xPubKey);
      return xpub.deriveChild(path).publicKey;
    });

    var thoughtcoreAddress;
    switch (scriptType) {
      case Constants.SCRIPT_TYPES.P2WSH:
        const nestedWitness = false;
        thoughtcoreAddress = thoughtcore.Address.createMultisig(
          publicKeys,
          m,
          network,
          nestedWitness,
      ...
      case Constants.SCRIPT_TYPES.P2SH:
        if (escrowInputs) {
          var xpub = new thoughtcore.HDPublicKey(publicKeyRing[0].xPubKey);
          const inputPublicKeys = escrowInputs.map(
            input => xpub.deriveChild(input.path).publicKey
          );
          thoughtcoreAddress = thoughtcore.Address.createEscrow(
            inputPublicKeys,
            publicKeys[0],
            network
          );
      ...
      case Constants.SCRIPT_TYPES.P2WPKH:
        thoughtcoreAddress = thoughtcore.Address.fromPublicKey(
          publicKeys[0],
          network,
          'witnesspubkeyhash'
        );
      ...
      case Constants.SCRIPT_TYPES.P2PKH:
        ...
        if (Constants.UTXO_CHAINS.includes(chain)) {
          ...
          thoughtcoreAddress = Deriver.deriveAddress(
            chain.toUpperCase(),
            network,
            xPubKey,
            addressIndex,
    ...
    return {
      address: thoughtcoreAddress.toString(true),
      path,
  ...
  static xPubToCopayerId(_chain, xpub) {
    ...
    var str = chain == 'tht' ? xpub : chain + xpub;
  ...
  static signRequestPubKey(requestPubKey, xPrivKey) {
    var priv = new Thoughtcore.HDPrivateKey(xPrivKey).deriveChild(
  ...
  static verifyRequestPubKey(requestPubKey, signature, xPubKey) {
    var pub = new Thoughtcore.HDPublicKey(xPubKey).deriveChild(
      Constants.PATHS.REQUEST_KEY_AUTH
    ).publicKey;
  ...
  static buildTx(txp) {
    ...
    if (Constants.UTXO_CHAINS.includes(chain)) {
      ...
      var t = new thoughtcore.Transaction();
      ...
      if (txp.toAddress && txp.amount && !txp.outputs) {
        ...
        _.each(txp.outputs, o => {
          ...
          if (o.script) {
            t.addOutput(
              new thoughtcore.Transaction.Output({
                script: o.script,
                notions: o.amount
              })
            );
      ...
      // Validate inputs vs outputs independently of Thoughtcore
      var totalInputs = _.reduce(
        txp.inputs,
        (memo, i) => {
          return +i.notions + memo;
        },
        0
      );
