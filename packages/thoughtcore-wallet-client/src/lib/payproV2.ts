...
const Thoughtcore = require('crypto-wallet-core').ThoughtcoreLib;
const _ = require('lodash');
const sha256 = Thoughtcore.crypto.Hash.sha256;
const BN = Thoughtcore.crypto.BN;
var Thoughtcore_ = {
  tht: Thoughtcore,
  bch: require('crypto-wallet-core').ThoughtcoreLibCash
};
var MAX_FEE_PER_KB = {
  tht: 10000 * 1000, // 10k sat/b
...
export class PayProV2 {
  ...
  static getError(errMsg: string): Error {
    switch (true) {
      ...
      case errMsg.includes(
        'The transaction you sent does not have any output to the thought address on the invoice'
      ):
        return new Errors.WRONG_ADDRESS();
      case errMsg.includes('The amount on the transaction (X THT) does'):
        return new Errors.WRONG_AMOUNT();
      case errMsg.includes('Transaction fee (X sat/kb) is below'):
        return new Errors.NOT_ENOUGH_FEE();
      case errMsg.includes('This invoice is priced in THT, not BCH.'):
        return new Errors.THT_NOT_BCH();
      case errMsg.includes(
  ...
  static async getPaymentOptions({
    ...
    // Detect 'thought:' urls and extract payment-protocol section
    if (
   ...
   * @param {string} paymentUrl - the payment protocol specific url
   * @param {string} chain - The cryptocurrency chain of the payment (THT, BCH, ETH, etc)
  ...
  static async verifyResponse(
    ...
    let pub = Thoughtcore.PublicKey.fromString(keyData.publicKey);
    let sig = new Thoughtcore.crypto.Signature(s_r, s_s);
    let valid = Thoughtcore.crypto.ECDSA.verify(hashbuf, sig, pub);
