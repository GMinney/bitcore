...
export class API extends EventEmitter {
  ...
  // Expose thoughtcore
  static Thoughtcore = CWC.ThoughtcoreLib;
  ...
  validateKeyDerivation(opts, cb) {
    ...
    var testHardcodedKeys = () => {
      ...
      var xpub = Thoughtcore.HDPublicKey.fromString(
    ...
    var testLiveKeys = () => {
      ...
      if (!xpriv) {
        xpriv = new Thoughtcore.HDPrivateKey(c.xPrivKey);
      }
      xpriv = xpriv.deriveChild(c.getBaseAddressDerivationPath());
      var xpub = new Thoughtcore.HDPublicKey(c.xPubKey);
  ...
  decryptBIP38PrivateKey(encryptedPrivateKeyBase58, passphrase, progressCallback, cb) {
    ...
    var privateKey = new Thoughtcore.PrivateKey(privateKeyWif);
    var address = privateKey.publicKey.toAddress().toString();
    var addrBuff = Buffer.from(address, 'ascii');
    var actualChecksum = Thoughtcore.crypto.Hash.sha256sha256(addrBuff)
      .toString('hex')
      .substring(0, 8);
    var expectedChecksum = Thoughtcore.encoding.Base58Check.decode(
  ...
  getBalanceFromPrivateKey(privateKey, chain, cb) {
    ...
    this.getUtxos(
      ...
      (err, utxos) => {
        ...
        return cb(null, _.sumBy(utxos, 'notions'));
      }
    );
  }
    ...
    async.waterfall(
      [
        ...
        (utxos, next) => {
          ...
          var amount = _.sumBy(utxos, 'notions') - fee;
  ...
  static _buildSecret(walletId, walletPrivKey, chain, network) {
    if (_.isString(walletPrivKey)) {
      walletPrivKey = Thoughtcore.PrivateKey.fromString(walletPrivKey);
    }
    var widHex = Buffer.from(walletId.replace(/-/g, ''), 'hex');
    var widBase58 = new Thoughtcore.encoding.Base58(widHex).toString();
    return (
  ...
  static parseSecret(secret) {
    ...
    try {
      ...
      var widHex = Thoughtcore.encoding.Base58.decode(widBase58).toString('hex');
      var walletId = split(widHex, [8, 12, 16, 20]).join('-');

      var walletPrivKey = Thoughtcore.PrivateKey.fromString(secretSplit[1]);
      var networkChar = secretSplit[2];
      var coin = secretSplit[3] || 'tht';
  ...
  _addSignaturesToThoughtcoreTxThought(txp, t, signatures, xpub) {
    $.checkState(
      txp.coin,
      'Failed state: txp.coin undefined at _addSignaturesToThoughtcoreTxThought'
    );
    $.checkState(
      txp.signingMethod,
      'Failed state: txp.signingMethod undefined at _addSignaturesToThoughtcoreTxThought'
    );

    var chain = txp.chain?.toLowerCase() || Utils.getChain(txp.coin); // getChain -> backwards compatibility
    const thoughtcore = Thoughtcore_[chain];
    if (signatures.length != txp.inputs.length)
      throw new Error('Number of signatures does not match number of inputs');

    let i = 0;
    const x = new thoughtcore.HDPublicKey(xpub);

    _.each(signatures, signatureHex => {
      try {
        const signature = thoughtcore.crypto.Signature.fromString(signatureHex);
        ...
        const s = {
          ...
          sigtype:
            ...
            thoughtcore.crypto.Signature.SIGHASH_ALL |
            thoughtcore.crypto.Signature.SIGHASH_FORKID,
          publicKey: pub
        };
  ...
  _addSignaturesToThoughtcoreTx(txp, t, signatures, xpub) {
    ...
    switch (chain.toLowerCase()) {
      ...
      case 'op':
        ...
        break;
      default:
        return this._addSignaturesToThoughtcoreTxThought(txp, t, signatures, xpub);
    }
  }
  ...
  _applyAllSignatures(txp, t) {
    ...
    _.each(sigs, x => {
      this._addSignaturesToThoughtcoreTx(txp, t, x.signatures, x.xpub);
    });
  }
  ...
  // *
  // * @param {string} chain - 'tht' (default) or 'bch'
  ...
  // * @param {string} opts.coin[='tht'] - The coin for this wallet (tht, bch).
  // * @param {string} opts.chain[='tht'] - The chain for this wallet (tht, bch).
  ...
  createWallet(walletName, copayerName, m, n, opts, cb) {
    ...
    var walletPrivKey = opts.walletPrivKey || new Thoughtcore.PrivateKey();
    ...
    var args = {
      ...
      m,
      n,
      pubKey: new Thoughtcore.PrivateKey(walletPrivKey).toPublicKey().toString(),
      chain,
      coin,
      network,
  ...
  // * @param {String} copayerName
  // * @param {Object} opts
  // * @param {string} opts.coin[='tht'] - The expected coin for this wallet (tht, bch).
  ...
  recreateWallet(cb) {
    ...
    this.getStatus(
      ...
      err => {
        ...
        var c = this.credentials;
        var walletPrivKey = Thoughtcore.PrivateKey.fromString(c.walletPrivKey);
  ...
  fetchPayPro(opts, cb) {
    ...
    PayPro.get(
      {
        url: opts.payProUrl,
        coin: this.credentials.coin || 'tht',
  ...
  /**
   * Create advertisement for thoughtnetwork app - (limited to marketing staff)
   * @param opts - options
   */
  ...
  /**
   * Get advertisements for thoughtnetwork app - (limited to marketing staff)
   * @param opts - options
  ...
  /**
   * Get advertisements for thoughtnetwork app, for specified country - (limited to marketing staff)
   * @param opts - options
  ...
  // * @param {Number} n
  // * @param {Object} opts
  // * @param {String} opts.coin (default 'tht')
  ...
  addAccess(opts, cb) {
    ...
    opts = opts || {};
    var requestPubKey = new Thoughtcore.PrivateKey(opts.requestPrivKey)
      .toPublicKey()
      .toString();
  ...
  // * @param {String} [opts.coin] - Coin (detault: 'tht')
  ...
  // * @param {number} opts.feePerKb - Optional. Specify the fee per KB (in notions).
  ...
  static upgradeMultipleCredentialsV1(oldCredentials) {
    ...
    if (newKeys.length > 0) {
      ...
      let credGroups = _.groupBy(newCrededentials, x => {
        ...
        let xpub = new Thoughtcore.HDPublicKey(x.xPubKey);
  ...
  static serverAssistedImport(opts, clientOpts, callback) {
    ...
    if (includeLegacyWallets) {
      const legacyOpts = [
        ...
        {
          // old THT/BCH  multisig wallets: /[44]/[0,145]'/
          nonCompliantDerivation: false,
          useLegacyCoinType: false,
