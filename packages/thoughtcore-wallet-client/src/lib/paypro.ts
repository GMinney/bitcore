...
export class PayPro {
  ...
  static _verify(requestUrl, headers, network, trustedKeys, callback) {
    ...
    let s_rBN = Thoughtcore.crypto.BN.fromBuffer(s_r);
    let s_sBN = Thoughtcore.crypto.BN.fromBuffer(s_s);

    let pub = Thoughtcore.PublicKey.fromString(keyData.publicKey);

    let sig = new Thoughtcore.crypto.Signature();
  ...
  static runRequest(opts, cb) {
    ...
    r.end((err, res) => {
      ...
      let hash = Thoughtcore.crypto.Hash.sha256(Buffer.from(body, 'utf8')).toString(
        'hex'
      );
  ...
  static get(opts, cb) {
    ...
    var thoughtcore = Thoughtcore_[coin];
    ...
    PayPro.runRequest(opts, function (err, data) {
      ...
      try {
        ret.toAddress = new thoughtcore.Address(data.outputs[0].address).toString(
          true
        );
      } catch (e) {
